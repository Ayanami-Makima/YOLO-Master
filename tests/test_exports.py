...
from tests import GENERIC_EXPORT_TASKS, MODEL, SOURCE
...
@pytest.mark.parametrize(
    ...
    [  # generate all combinations except for exclusion cases
        ...
        for task, dynamic, quantize, batch, nms, end2end in product(
            GENERIC_EXPORT_TASKS, [True, False], [8, 16], [1, 2], [True, False], [True]
        )
...
@pytest.mark.parametrize(
    ...
    [  # generate all combinations except for exclusion cases
        ...
        for task, dynamic, batch, simplify, nms, end2end in product(
            GENERIC_EXPORT_TASKS, [True, False], [1, 2], [True, False], [True, False], [True, False]
        )
...
@pytest.mark.parametrize(
    ...
    [  # generate all combinations except for exclusion cases
        ...
        for task, dynamic, batch, nms, end2end in product(
            GENERIC_EXPORT_TASKS, [False, True], [1, 2], [True, False], [True, False]
        )
        if not ((task == "classify" and nms) or (end2end and nms))
    ],
)
...
@pytest.mark.parametrize(
    ...
    [  # generate all combinations except for exclusion cases
        ...
        for task, dynamic, quantize, nms, batch, end2end in product(
            GENERIC_EXPORT_TASKS, [True, False], [8, 16], [True, False], [1], [True, False]
        )
...
@pytest.mark.parametrize(
    ...
    [  # generate all combinations except for exclusion cases
        (task, quantize, batch, end2end)
        for task, quantize, batch, end2end in product(GENERIC_EXPORT_TASKS, [8, 16], [1, 2], [True, False])
    ],
)
...
@pytest.mark.slow
@pytest.mark.skipif(not TORCH_2_0, reason="NCNN inference causes segfault on PyTorch<2.0")
@pytest.mark.parametrize("task, quantize, batch", list(product(GENERIC_EXPORT_TASKS, [16], [1])))
...
@pytest.mark.parametrize(
    "task, quantize",
    [(task, quantize) for task in GENERIC_EXPORT_TASKS for quantize in (None, 8, "w8a16", "w8a32")],
)
...
@pytest.mark.skipif(not checks.IS_PYTHON_MINIMUM_3_10 or not TORCH_2_9, reason="Requires Python>=3.10 and Torch>=2.9.0")
@pytest.mark.skipif(WINDOWS, reason="Skipping test on Windows")
@pytest.mark.parametrize("task", GENERIC_EXPORT_TASKS)
