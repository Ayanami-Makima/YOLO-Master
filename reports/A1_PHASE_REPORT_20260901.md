...
> 阶段：P0 / P1（截至 2026-09-01）｜作者：Ayanami-Makima｜状态：阶段报告
...
P0 已具备 checkpoint 身份、精度、真实 NMS 调用与延迟证据；P1 已形成 initializer 等价性、preflight、冻结状态、路由、残差活性、正式准入、断点恢复和最终 checkpoint 的完整证据链。r28 的 12 个正式单元均可由锁定数据列表、种子、协议和 checkpoint 追溯。

## 6. 下一步计划

### 近期

1. 补充 r28 最终权重在完整 val5000 上的统一路由分布诊断，包括专家选择率、归一化熵和样本尺度下的覆盖情况。
2. 在同权重、同设备、同 batch 条件下补齐 A/B/C/D 的推理延迟、显存峰值和吞吐量，形成“精度—速度—资源”联合评价。
