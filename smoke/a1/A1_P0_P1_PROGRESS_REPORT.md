...
- **P0：建立可信双路径基线。** 完成官方预训练模型的完整 COCO 验证、真实 NMS 调用审计和 CPU/GPU 测量，为后续对照提供统一起点。
- **P1：实现并完成等预算三种子实验。** 零初始化残差适配器保留预训练功能，完成 12 个正式单元及冻结、恢复、导出和资源审计，量化 MoE 与 End-to-End 的独立效应。
- **P2：形成机制验证与干预闭环。** 已验证原生 detach 是 one-to-one 检测损失无法直接更新共享 factor/router 的原因；梯度桥恢复了该通路。进一步量化 Router/分发开销、候选匹配冲突与固定阈值漏检现象，并通过配对复评、bootstrap 和 seg pilot 完成证据交付。

上述发现对应各自明确的测量范围：梯度恢复是已验证的机制结果；其 AP 效果仍以配对实验为准，目前未获得稳定提升。以下保留各轮历史记录和原始数值，供审阅追溯。
