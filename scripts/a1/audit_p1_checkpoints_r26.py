#!/usr/bin/env python3
"""Audit every seed/cell r26 checkpoint against its original initializer."""
...
from run_p1_bn_frozen_r26 import (
    R26_CLEAN_AUX_POLICY,
    R26_DENSE_AUX_POLICY,
    R26_GAIN_POLICY,
    R26_ROUTING_SEMANTICS,
)
...
def runtime_policy(run_dir: Path, *, routed_cell: bool, stage: str, seed: int) -> dict:
    """Verify the persisted r26 freeze, optimizer, and routing policy."""
    path = run_dir / "p1_runtime_policy.json"
    payload = json.loads(path.read_text(encoding="utf-8")) if path.is_file() else {}
    reasons = []
    if payload.get("routing_semantics") != R26_ROUTING_SEMANTICS:
        reasons.append("routing semantics mismatch")
    expected_aux = R26_CLEAN_AUX_POLICY if routed_cell else R26_DENSE_AUX_POLICY
    ...
    for index, module in enumerate(routed):
        ...
        if module.get("routing_aux_semantics") != R26_CLEAN_AUX_POLICY["runtime_semantics"]:
    ...
    if (
        len(gain_groups) != 1
        or not math.isclose(float(gain_groups[0].get("initial_lr", -1)), R26_GAIN_POLICY["lr"], abs_tol=1e-12)
...
def main() -> None:
    ...
    if protocol.get("schema_version") != 8 or protocol.get("experiment_tag") != "r26":
        raise ValueError("r26 checkpoint audit requires an r26 schema-8 protocol")
