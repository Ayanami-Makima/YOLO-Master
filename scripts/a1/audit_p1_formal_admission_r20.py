#!/usr/bin/env python3
"""Compose the fail-closed r20 formal-admission evidence."""
...
def validate_implementation(protocol: dict[str, Any]) -> None:
    ...
    require(not dirty, "r20 worktree is dirty at formal admission")
...
def validate_stage_execution(protocol: dict[str, Any], protocol_path: Path, stage: str, cells: str) -> dict[str, str]:
    ...
    require(status.get("schema_version") == 6 and status.get("stage") == stage, f"{stage} status schema drift")
...
def validate_checkpoint_audit(
    ...
    require(payload.get("schema_version") == 6, f"{stage} checkpoint audit schema drift")
...
def validate_routing(payload: dict[str, Any], protocol: dict[str, Any], probe: dict[str, Any]) -> int:
    require(payload.get("schema_version") == 6, "routing audit schema drift")
    require(
...
def validate_initializers(protocol: dict[str, Any], protocol_path: Path) -> tuple[list[dict[str, str]], bool]:
    ...
    for seed in SEEDS:
        ...
        require(manifest.get("schema_version") == 6 and manifest.get("status") == "passed", f"seed{seed} manifest failed")
        ...
        for cell in CELLS:
            ...
            clean_aux_routers = item.get("r20_clean_aux_routers")
            if cell in "ab":
...
def main() -> None:
    ...
    protocol = read_json(protocol_path, "r20 protocol")
