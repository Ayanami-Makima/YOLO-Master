#!/usr/bin/env python3
"""Audit every seed/cell r20 checkpoint against its original initializer."""
...
from run_p1_bn_frozen_r20 import (
    R20_CLEAN_AUX_POLICY,
    R20_DENSE_AUX_POLICY,
    R20_GAIN_POLICY,
    R20_ROUTING_SEMANTICS,
)
...
def runtime_policy(run_dir: Path, *, routed_cell: bool, stage: str, seed: int) -> dict:
    """Verify the persisted r20 freeze, optimizer, and routing policy."""
    path = run_dir / "p1_runtime_policy.json"
    payload = json.loads(path.read_text(encoding="utf-8")) if path.is_file() else {}
    reasons = []
    if payload.get("routing_semantics") != R20_ROUTING_SEMANTICS:
        reasons.append("routing semantics mismatch")
    expected_aux = R20_CLEAN_AUX_POLICY if routed_cell else R20_DENSE_AUX_POLICY
    ...
    for index, module in enumerate(routed):
        ...
        if module.get("routing_aux_semantics") != R20_CLEAN_AUX_POLICY["runtime_semantics"]:
    ...
    if (
        len(gain_groups) != 1
        or not math.isclose(float(gain_groups[0].get("initial_lr", -1)), R20_GAIN_POLICY["lr"], abs_tol=1e-12)
...
def main() -> None:
    ...
    if protocol.get("schema_version") != 6 or protocol.get("experiment_tag") != "r20":
        raise ValueError("r20 checkpoint audit requires an r20 schema-6 protocol")
