#!/usr/bin/env python3
"""Generate the immutable A1 P1 medium-COCO r26 residual-factor protocol.

r26 preserves r23's model, routing, initializer, freeze, runtime-source binding,
...
import yaml
from p1_r26_runtime import RUNTIME_ATTESTATION

# isort: split

from p1_r26_integrity import data_list_content_signature
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--experiment-tag", default="r26")
...
def require_locked_baseline_ancestor(repo: Path) -> None:
    """Fail unless the r26 implementation descends from the locked official baseline."""
...
def validate_destinations(repo: Path, run_root: Path, output: Path, tag: str) -> None:
    if tag != "r26":
        raise ValueError(f"r26 protocol requires experiment-tag r26, got {tag!r}")
    if repo.name != "YOLO-Master-r26-medium":
        raise ValueError(f"r26 requires isolated worktree YOLO-Master-r26-medium, got {repo}")
    expected_output = repo / "configs/a1/p1_factorial_medium_r26"
    if not same_path(output, expected_output):
        raise ValueError(f"r26 config directory must be {expected_output}, got {output}")
    if run_root.name != "p1_factorial_medium_r26":
        raise ValueError(f"r26 run root must end in p1_factorial_medium_r26, got {run_root}")
...
def calibration_images(formal_data: Path) -> tuple[list[str], Path]:
    ...
    if not isinstance(train, str):
        raise TypeError("r26 requires formal train to be one explicit image-list file")
    ...
    if len(lines) < 512:
        raise ValueError(f"r26 BN calibration needs 512 train images, found {len(lines)}")
    resolved: list[str] = []
...
def data_registry_entry(
    ...
    for split, expected_count in (("train", expected_train), ("val", expected_val)):
        ...
        if not isinstance(entry, str):
            raise TypeError(f"r26 requires {data_yaml} {split} to be one explicit image-list file")
...
def main() -> None:
    ...
    if python != EXPECTED_PYTHON:
        raise ValueError(f"r26 requires the exact virtualenv launcher {EXPECTED_PYTHON}, got {python}")
    if RUNTIME_ATTESTATION["interpreter"]["executable"] != str(python):
        raise RuntimeError("r26 protocol generator was not launched by the registered virtualenv interpreter")
    if RUNTIME_ATTESTATION["interpreter"]["virtualenv"] is not True:
        raise RuntimeError("r26 protocol generation requires an active virtualenv interpreter")
    forbid_r19_artifact(checkpoint, "source checkpoint")
    if checkpoint != OFFICIAL_CHECKPOINT.resolve():
        raise ValueError(f"r26 must start from the locked official checkpoint {OFFICIAL_CHECKPOINT}, got {checkpoint}")
    if sha256(checkpoint) != OFFICIAL_CHECKPOINT_SHA256:
        raise ValueError("official yolo26n.pt SHA-256 mismatch")
    branch = git_output(repo, "branch", "--show-current")
    if branch != "a1-p1-medium-r26":
        raise ValueError(f"r26 worktree branch must be a1-p1-medium-r26, got {branch!r}")
    require_locked_baseline_ancestor(repo)
    dirty = git_output(repo, "status", "--porcelain")
    if dirty:
        raise ValueError("implementation worktree must be clean before generating the r26 protocol")
    ...
    implementation_paths = (
        ...
        *moe_runtime_paths,
        ...
        repo / "scripts/a1/prepare_p1_residual_factor_r26.py",
        repo / "scripts/a1/prepare_p1_medium_data_r26.py",
        repo / "scripts/a1/p1_r26_integrity.py",
        repo / "scripts/a1/p1_r26_runtime.py",
        repo / "scripts/a1/prepare_p1_factorial.py",
        repo / "scripts/a1/build_p1_residual_factor_initializers_r26.py",
        repo / "scripts/a1/build_p1_residual_factor_initializers.py",
        repo / "scripts/a1/run_p1_bn_frozen_r26.py",
        repo / "scripts/a1/run_p1_factorial_multiseed_2gpu_r26.py",
        repo / "scripts/a1/audit_p1_checkpoints_r26.py",
        repo / "scripts/a1/audit_p1_residual_factor_checkpoints.py",
        repo / "scripts/a1/audit_p1_routing_r26.py",
        repo / "scripts/a1/audit_p1_preflight_routing.py",
        repo / "scripts/a1/audit_p1_routing.py",
        repo / "scripts/a1/evaluate_p1_matrix.py",
        repo / "scripts/a1/audit_p1_residual_activity_r26.py",
        repo / "scripts/a1/audit_p1_formal_admission_r26.py",
        repo / "scripts/a1/verify_p1_r26_migration.py",
        repo / "tests/test_p1_residual_factor.py",
        repo / "tests/test_moe_router_boundaries.py",
        repo / "tests/test_peft_optimizer_policy.py",
        repo / "tests/test_p1_r26_protocol.py",
    )
    ...
    protocol: dict[str, Any] = {
        "schema_version": 8,
        "name": "A1 P1 medium-COCO equal-budget 2x2 factorial r26 residual-factor",
        "experiment_tag": "r26",
        ...
        "r26_medium_coco_extension": {
            "scope": "medium-COCO formal budget plus user-authorized shared-GPU execution; authorized before any r26 training",
        ...
        "routing_probe_policy": "C/D x 3 seeds start independently from original r26 initializers; discard all weights",
