#!/usr/bin/env python3
"""Evaluate P2-E r3 assigner-budget pilot checkpoints on fixed val512."""
...
RUNS = (
    "b_topk2_1_seed260829_5ep",
    "b_topk2_2_seed260829_5ep",
    "d_topk2_1_seed260829_5ep",
    "d_topk2_2_seed260829_5ep",
)
...
def main() -> None:
    ...
    if evidence_path.is_file():
        ...
        evidence = {
            "schema": "a1-p2-e2e-assigner-budget-r3-eval/v1",
            "status": "running",
    ...
    for run_name in RUNS:
        if run_name in evidence["runs"] and evidence["runs"][run_name].get("metrics"):
            print(f"[p2-e2e-r3-eval] resume: {run_name} already completed", flush=True)
            continue
        checkpoint = args.project / run_name / "weights" / f"{args.checkpoint_kind}.pt"
        if not checkpoint.is_file():
            raise FileNotFoundError(checkpoint)
        print(f"[p2-e2e-r3-eval] {run_name}: {checkpoint}", flush=True)
