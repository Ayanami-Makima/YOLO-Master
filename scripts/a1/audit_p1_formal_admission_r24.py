#!/usr/bin/env python3
"""Compose the fail-closed r24 formal-admission evidence."""
...
def validate_implementation(protocol: dict[str, Any]) -> None:
    ...
    require(not dirty, "r24 worktree is dirty at formal admission")
...
def validate_stage_execution(protocol: dict[str, Any], protocol_path: Path, stage: str, cells: str) -> dict[str, str]:
    ...
    runtime_relative = "scripts/a1/p1_r24_runtime.py"
    require(
...
def validate_initializers(protocol: dict[str, Any], protocol_path: Path) -> tuple[list[dict[str, str]], bool]:
    ...
    for seed in SEEDS:
        ...
        for cell in CELLS:
            ...
            clean_aux_routers = item.get("r24_clean_aux_routers")
            if cell in "ab":
...
def main() -> None:
    ...
    protocol = read_json(protocol_path, "r24 protocol")
    ...
    require(
        ...
        "r24 must not reuse r23 initializer or trained weights",
    )
