#!/usr/bin/env python3
"""Apply the immutable fixed-512 hard Top-2 gate to all r20 C/D probes.
    ...
    for name, module in model.model.named_modules():
        ...
        if routing is not None and hasattr(routing, "num_experts") and hasattr(routing, "top_k"):
            if getattr(routing, "p1_balance_on_clean_routes", False) is not True:
                raise ValueError(f"r20 clean-aux flag missing: {name}.routing")
            if getattr(module, "routing_aux_semantics", None) != R20_CLEAN_AUX_POLICY["runtime_semantics"]:
                raise ValueError(f"r20 routing-aux semantics mismatch: {name}.routing")
...
def main() -> None:
    ...
    if protocol.get("schema_version") != 6 or protocol.get("experiment_tag") != "r20":
        raise ValueError("r20 routing audit requires an r20 schema-6 protocol")
    ...
    if output.exists():
        raise FileExistsError("refusing to overwrite r20 routing evidence")
