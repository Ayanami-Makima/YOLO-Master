#!/usr/bin/env python3
"""Verify r25 implementation identity, A/B invariance, and C/D policy parity."""
...
from run_p1_bn_frozen_r25 import R25_CLEAN_AUX_POLICY
...
def verify_git_lineage(repo: Path, protocol: dict[str, Any]) -> dict[str, str]:
    ...
    require(not dirty, "r25 worktree is dirty during migration audit")
...
def main() -> None:
    ...
    # r25 deliberately recalibrates factor BatchNorm on the first 512 full-COCO
    ...
    for key in semantic_keys:
        ...
        require(equal, f"r25 semantic drift from r23: {key}")
    ...
    for key in (
        ...
        require(protocol["gpu_policy"].get(key) == r23["gpu_policy"].get(key), f"r25 GPU schedule drift: {key}")
    ...
    for cell in "abcd":
        ...
        for key in ("moe", "end2end", "sha256"):
            require(current.get(key) == previous.get(key), f"r25 config semantic drift: {cell}/{key}")
    for key in ("images", "batch", "imgsz", "augment", "shuffle", "grad", "post_calibration_training_policy"):
        require(protocol["batch_norm"].get(key) == r23["batch_norm"].get(key), f"r25 BN policy drift: {key}")
    ...
    for seed in SEEDS:
        ...
        for cell in "ab":
            r25_path = Path(protocol["run_root"]) / "initializers" / f"seed{seed}" / f"{cell}_residual_factor_init.pt"
            require(r25_path.is_file(), f"missing A/B initializer: {seed}/{cell}")
            ...
            require(passed, f"r25 A/B invariance failed: {seed}/{cell}")
            comparisons[str(seed)][cell] = {
                "r25": str(r25_path),
                "r25_sha256": sha256(r25_path),
        ...
        for cell in "cd":
            ...
            require(
                all(item[3] and item[4] == R25_CLEAN_AUX_POLICY["runtime_semantics"] for item in signatures[cell]),
