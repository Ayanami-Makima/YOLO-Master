#!/usr/bin/env python3
"""Build one seed's calibrated, function-preserving A1 P1 r19 initializers."""
...
def calibrate_factor_batch_norm(
    ...
    for _, core in routed_cores(model):
        ...
        if not groups or not groups[0]:
            raise ValueError("r19 expected BatchNorm-bearing cloned experts")
        if any(len(group) != len(groups[0]) for group in groups):
            raise ValueError("r19 expert BatchNorm structures are not aligned")
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("r19 initializer calibration requires CUDA")
