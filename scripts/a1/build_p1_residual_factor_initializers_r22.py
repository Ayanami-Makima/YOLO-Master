#!/usr/bin/env python3
"""Build one seed's calibrated, function-preserving A1 P1 r22 initializers."""
...
def clean_aux_router_report(module: nn.Module, *, expected: bool) -> list[dict]:
    """Attest the r22 clean-aux flag without changing dispatch or weights."""
    reports = []
    for name, core in routed_cores(module):
        ...
        if expected and reports[-1]["routing_aux_semantics"] != EXPECTED_CLEAN_AUX_POLICY["runtime_semantics"]:
            raise ValueError(f"{name}: r22 routing-aux semantics mismatch")
...
def calibrate_factor_batch_norm(
    ...
    for _, core in routed_cores(model):
        ...
        if not groups or not groups[0]:
            raise ValueError("r22 expected BatchNorm-bearing cloned experts")
        if any(len(group) != len(groups[0]) for group in groups):
            raise ValueError("r22 expert BatchNorm structures are not aligned")
    ...
    for cell in CELLS:
        ...
        for layer_index in FACTOR_LAYERS:
            ...
            layers[str(layer_index)] = {
                ...
                "r22_clean_aux_routers": clean_aux_router_report(adapter.factor, expected=is_moe),
            }
...
def main() -> None:
    ...
    if protocol.get("schema_version") != 8 or protocol.get("experiment_tag") != "r22":
        raise ValueError("r22 initializer builder requires an r22 schema-8 protocol")
    if tuple(protocol.get("seeds", ())) != EXPECTED_SEEDS:
        raise ValueError(f"r22 seed registry drift: {protocol.get('seeds')}")
    if protocol.get("routing", {}).get("auxiliary_objective") != EXPECTED_CLEAN_AUX_POLICY:
        raise ValueError("r22 clean-aux policy drift")
    ...
    if set(protocol.get("configs", {})) != set(CELLS):
        raise ValueError("r22 model-config registry must contain exactly A/B/C/D")
    for cell in CELLS:
        ...
        if not config_path.is_file() or sha256(config_path) != config.get("sha256"):
            raise ValueError(f"r22 model config hash drift: {cell}/{config_path}")
    ...
    for relative, expected_sha in protocol.get("implementation", {}).items():
        ...
        if not implementation_path.is_file() or sha256(implementation_path) != expected_sha:
            raise ValueError(f"r22 implementation hash drift: {relative}")
    ...
    if not pilot_yaml.is_file() or sha256(pilot_yaml) != pilot.get("sha256"):
        raise ValueError("r22 pilot YAML hash drift")
    for split, expected_count in (("train", 5000), ("val", 512)):
        ...
        if not list_path.is_file() or sha256(list_path) != item.get("sha256"):
            raise ValueError(f"r22 pilot {split} image-list hash drift")
        ...
        if count != expected_count or item.get("images") != expected_count:
            raise ValueError(f"r22 pilot {split} image count drift")
    ...
    if (
        ...
        raise ValueError(f"r22 must start from official yolo26n.pt, never r19 weights: {checkpoint}")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("r22 initializer calibration requires CUDA")
    ...
    manifest["a_b_dense_clean_aux_not_applicable"] = all(
        not manifest["cells"][cell]["r22_clean_aux_routers"] for cell in "ab"
    )
    manifest["c_d_clean_aux_policy_parity"] = (
        manifest["cells"]["c"]["r22_clean_aux_routers"]
        == manifest["cells"]["d"]["r22_clean_aux_routers"]
    )
    if not manifest["a_b_dense_clean_aux_not_applicable"] or not manifest["c_d_clean_aux_policy_parity"]:
        raise ValueError("r22 dense invariance or C/D clean-aux policy parity failed")
