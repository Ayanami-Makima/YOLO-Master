...
PROJECT=$FORMAL/p2_e2e_loss_r2_corrected
...
run_one "$FORMAL/initializers/seed260829/b_residual_factor_init.pt" \
  b_gain1p0_seed260829_5ep 1.0 p2_e2e_loss_r2_corrected_b_gain1p0.log
run_one "$FORMAL/initializers/seed260829/b_residual_factor_init.pt" \
  b_gain1p2_seed260829_5ep 1.2 p2_e2e_loss_r2_corrected_b_gain1p2.log
run_one "$FORMAL/initializers/seed260829/d_residual_factor_init.pt" \
  d_gain1p0_seed260829_5ep 1.0 p2_e2e_loss_r2_corrected_d_gain1p0.log
run_one "$FORMAL/initializers/seed260829/d_residual_factor_init.pt" \
  d_gain1p2_seed260829_5ep 1.2 p2_e2e_loss_r2_corrected_d_gain1p2.log

cd "$ROOT"
"$PY" "$EVAL" --project "$PROJECT" --data "$DATA" \
  --output "$PROJECT/eval_fixed_val512" --device cuda:1 \
  > "$FORMAL/p2_e2e_loss_r2_corrected_eval.log" 2>&1
printf '%s\n' "P2-E r2-c sequence completed: $(date -Is)" > "$FORMAL/p2_e2e_loss_r2_corrected.sequence.done"
