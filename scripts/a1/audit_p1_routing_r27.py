#!/usr/bin/env python3
"""Apply the A1-aligned full-val5000 hard Top-2 diagnostics to r27 C/D probes.
    ...
    for name, module in model.model.named_modules():
        ...
        if routing is not None and hasattr(routing, "num_experts") and hasattr(routing, "top_k"):
            if getattr(routing, "p1_balance_on_clean_routes", False) is not True:
                raise ValueError(f"r27 clean-aux flag missing: {name}.routing")
            if getattr(module, "routing_aux_semantics", None) != R27_CLEAN_AUX_POLICY["runtime_semantics"]:
                raise ValueError(f"r27 routing-aux semantics mismatch: {name}.routing")
...
def main() -> None:
    ...
    if protocol.get("schema_version") != 8 or protocol.get("experiment_tag") != "r27":
        raise ValueError("r27 routing audit requires an r27 schema-8 protocol")
    ...
    output = Path(protocol["run_root"]) / "audits" / "routing" / "hard_top2_5000.json"
    if output.exists():
        raise FileExistsError("refusing to overwrite r27 routing evidence")
    data = Path(protocol["data"]["formal"]["path"])
    paths = resolve_images(data, None, EXPECTED_IMAGES)
    if len(paths) != EXPECTED_IMAGES:
        raise ValueError(f"routing gate requires exactly {EXPECTED_IMAGES} images, got {len(paths)}")
    ...
    report = {
        ...
        "admission_policy": "full-val5000 A1 collapse gate plus exact dead-expert diagnostic; residual activity remains independently measured on fixed pilot val512",
        "seeds": {},
    }
