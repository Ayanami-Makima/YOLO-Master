#!/usr/bin/env python3
"""Build one seed's calibrated, function-preserving A1 P1 r26 initializers."""
...
def clean_aux_router_report(module: nn.Module, *, expected: bool) -> list[dict]:
    """Attest the r26 clean-aux flag without changing dispatch or weights."""
    reports = []
    for name, core in routed_cores(module):
        ...
        if expected and reports[-1]["routing_aux_semantics"] != EXPECTED_CLEAN_AUX_POLICY["runtime_semantics"]:
            raise ValueError(f"{name}: r26 routing-aux semantics mismatch")
...
def calibrate_factor_batch_norm(
    ...
    for _, core in routed_cores(model):
        ...
        if not groups or not groups[0]:
            raise ValueError("r26 expected BatchNorm-bearing cloned experts")
        if any(len(group) != len(groups[0]) for group in groups):
            raise ValueError("r26 expert BatchNorm structures are not aligned")
    ...
    for cell in CELLS:
        ...
        for layer_index in FACTOR_LAYERS:
            ...
            layers[str(layer_index)] = {
                ...
                "r26_clean_aux_routers": clean_aux_router_report(adapter.factor, expected=is_moe),
            }
...
def main() -> None:
    ...
    if protocol.get("schema_version") != 8 or protocol.get("experiment_tag") != "r26":
        raise ValueError("r26 initializer builder requires an r26 schema-8 protocol")
    if tuple(protocol.get("seeds", ())) != EXPECTED_SEEDS:
        raise ValueError(f"r26 seed registry drift: {protocol.get('seeds')}")
    if protocol.get("routing", {}).get("auxiliary_objective") != EXPECTED_CLEAN_AUX_POLICY:
        raise ValueError("r26 clean-aux policy drift")
    ...
    if set(protocol.get("configs", {})) != set(CELLS):
        raise ValueError("r26 model-config registry must contain exactly A/B/C/D")
    for cell in CELLS:
        ...
        if not config_path.is_file() or sha256(config_path) != config.get("sha256"):
            raise ValueError(f"r26 model config hash drift: {cell}/{config_path}")
    ...
    for relative, expected_sha in protocol.get("implementation", {}).items():
        ...
        if not implementation_path.is_file() or sha256(implementation_path) != expected_sha:
            raise ValueError(f"r26 implementation hash drift: {relative}")
    expected_data = {
    ...
    for label, split_counts in expected_data.items():
        ...
        if not data_yaml.is_file() or sha256(data_yaml) != registry.get("sha256"):
            raise ValueError(f"r26 {label} YAML hash drift")
        for split, expected_count in split_counts.items():
            ...
            if not list_path.is_file() or sha256(list_path) != item.get("sha256"):
                raise ValueError(f"r26 {label} {split} image-list hash drift")
            ...
            if count != expected_count or item.get("images") != expected_count:
                raise ValueError(f"r26 {label} {split} image count drift")
    ...
    if (
        ...
        raise ValueError(f"r26 must start from official yolo26n.pt, never r19 weights: {checkpoint}")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("r26 initializer calibration requires CUDA")
    ...
    manifest["a_b_dense_clean_aux_not_applicable"] = all(
        not manifest["cells"][cell]["r26_clean_aux_routers"] for cell in "ab"
    )
    manifest["c_d_clean_aux_policy_parity"] = (
        manifest["cells"]["c"]["r26_clean_aux_routers"]
        == manifest["cells"]["d"]["r26_clean_aux_routers"]
    )
    if not manifest["a_b_dense_clean_aux_not_applicable"] or not manifest["c_d_clean_aux_policy_parity"]:
        raise ValueError("r26 dense invariance or C/D clean-aux policy parity failed")
