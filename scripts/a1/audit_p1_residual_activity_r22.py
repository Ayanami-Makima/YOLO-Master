#!/usr/bin/env python3
"""Audit r22 residual-factor activity on the fixed pilot-validation sample.

This diagnostic treats every routing-probe checkpoint as immutable. It evaluates
the exact C/D ``last.pt`` files registered by the r22 protocol and measures, for
layers 4, 6, and 8,
...
def validate_protocol(protocol_path: Path, protocol: dict[str, Any]) -> tuple[list[int], Path]:
    """Validate r22 identity, hash-locked inputs, and C/D probe requests."""
    require(protocol.get("schema_version") == EXPECTED_PROTOCOL_SCHEMA, "protocol schema is not r22 schema 8")
    require(protocol.get("experiment_tag") == "r22", "protocol experiment_tag is not r22")
    require(protocol.get("primary_checkpoint") == "last.pt", "r22 primary checkpoint must be last.pt")
    ...
    require(repo.is_dir(), f"registered r22 repository is missing: {repo}")
...
def disable_routing_noise(model: torch.nn.Module) -> list[dict[str, Any]]:
    ...
    for name, module in model.named_modules():
        ...
        require(
            getattr(module, "routing_aux_semantics", None) == R22_CLEAN_AUX_POLICY["runtime_semantics"],
