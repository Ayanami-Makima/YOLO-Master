#!/usr/bin/env python3
"""Compose the fail-closed r27 formal-admission evidence."""
...
from p1_r27_runtime import RUNTIME_ATTESTATION, assert_protocol_runtime

# isort: split

from p1_r27_integrity import verify_registered_data_content
...
TWO_JOB_FREE_MEMORY_MIB = 22000


def capacity_for_free_memory(free_mib: int) -> int:
    if free_mib >= TWO_JOB_FREE_MEMORY_MIB:
        return 2
    if free_mib >= ONE_JOB_FREE_MEMORY_MIB:
        return 1
    return 0


def expected_stage_schedule(
    stage: str, capacities: dict[str, int]
) -> tuple[tuple[tuple[int, str, str], ...], ...]:
    cells = "cd" if stage == "routing_probe" else "abcd"
    pending = [(seed, cell) for seed in SEEDS for cell in cells]
    waves = []
    while pending:
        wave = []
        for gpu in GPU_INDICES:
            for _ in range(capacities.get(gpu, 0)):
                if pending:
                    seed, cell = pending.pop(0)
                    wave.append((seed, cell, gpu))
        require(bool(wave), "recorded GPU capacities cannot schedule a job")
...
def validate_implementation(protocol: dict[str, Any]) -> None:
    ...
    require(not dirty, "r27 worktree is dirty at formal admission")
...
def validate_stage_execution(protocol: dict[str, Any], protocol_path: Path, stage: str, cells: str) -> dict[str, str]:
    ...
    child_runtime = status.get("child_runtime_preflight", {})
    require(child_runtime.get("passed") is True, f"{stage} child runtime preflight did not pass")
    require(
        child_runtime.get("python") == protocol["runtime_binding"]["interpreter"]["executable"],
        f"{stage} child interpreter drift",
    )
    require(child_runtime.get("attestation") == RUNTIME_ATTESTATION, f"{stage} child attestation drift")
    runtime_relative = "scripts/a1/p1_r27_runtime.py"
    require(
        child_runtime.get("runtime_script_sha256") == protocol["implementation"][runtime_relative],
        f"{stage} runtime bootstrap SHA drift",
    )
    require(status.get("schema_version") == 8 and status.get("stage") == stage, f"{stage} status schema drift")
    ...
    shared_policy = protocol.get("gpu_policy", {}).get("co_location", {})
    require(shared_policy.get("authorized") is True, f"{stage} shared-GPU policy is not authorized")
    require(
        shared_policy.get("one_job_minimum_free_memory_mib") == ONE_JOB_FREE_MEMORY_MIB
        and shared_policy.get("two_jobs_minimum_free_memory_mib") == TWO_JOB_FREE_MEMORY_MIB
        and shared_policy.get("maximum_jobs_per_gpu") == 2,
        f"{stage} shared-GPU capacity thresholds drift",
    )
    require(status.get("shared_gpu_resource_policy") == shared_policy, f"{stage} resource policy drift")
    runner = status.get("runner", {})
    relative_runner = "scripts/a1/run_p1_bn_frozen_r27.py"
    ...
    capacities = {index: capacity_for_free_memory(inventory[index].get("memory_free_mib", 0)) for index in GPU_INDICES}
    require(sum(capacities.values()) >= 1, f"{stage} started with no admissible GPU capacity")
    require(status.get("gpu_capacity_by_physical_index") == capacities, f"{stage} GPU capacity drift")
    require(
    ...
    expected_waves = expected_stage_schedule(stage, capacities)
    ...
    for wave, expected_assignments in zip(waves, expected_waves):
        ...
        for gpu in GPU_INDICES:
            require(
                sum(job_gpu == gpu for _, _, job_gpu in expected_assignments) <= capacities[gpu],
                f"{stage} exceeded GPU{gpu} concurrency capacity",
            )
...
def validate_checkpoint_audit(
    ...
    require(payload.get("schema_version") == 8, f"{stage} checkpoint audit schema drift")
...
def validate_routing(payload: dict[str, Any], protocol: dict[str, Any], probe: dict[str, Any]) -> int:
    require(payload.get("schema_version") == 8, "routing audit schema drift")
    require(
    ...
    require(Path(payload.get("data", "")).resolve() == Path(protocol["data"]["formal"]["path"]).resolve(), "routing data drift")
    images = payload.get("images", [])
    require(isinstance(images, list) and len(images) == 5000, "routing audit must bind exactly 5000 images")
    val_list = Path(protocol["data"]["formal"]["lists"]["val"]["path"])
    expected_paths = []
    ...
    require(len(expected_paths) == 5000, "formal validation list is not exactly 5000 images")
    ...
    for seed in SEEDS:
        ...
        for cell in "cd":
            ...
            for name, module in modules.items():
                ...
                require(module.get("top_k") == 2 and module.get("images") == 5000, f"routing shape drift: {name}")
                require(
                ...
                require(sum(counts) == 10000, f"routing selection total drift: {name}")
                require(
                    module.get("selections") == module.get("expected_selections") == 10000,
                    f"serialized routing selection total drift: {name}",
                )
                dead = [index for index, value in enumerate(counts) if value == 0]
                maximum = max(value / 5000 for value in counts)
                shares = [value / 10000 for value in counts]
                entropy = -sum(value * math.log(value) for value in shares if value > 0) / math.log(num_experts)
                require(
                    module.get("dead_experts_on_sample") == dead,
                    f"dead-expert diagnostic was not derived from counts: {seed}/{cell}/{name}",
                )
                diagnostic = module.get("gate", {}).get("dead_expert_diagnostic", {})
                require(
                    diagnostic
                    == {
                        "target": 0,
                    ...
                    f"dead-expert diagnostic drift: {seed}/{cell}/{name}",
                )
                require(
...
def validate_residual(
    ...
    require(sample.get("count") == 512, "residual audit must bind exactly 512 pilot images")
...
def validate_initializers(protocol: dict[str, Any], protocol_path: Path) -> tuple[list[dict[str, str]], bool]:
    ...
    for seed in SEEDS:
        ...
        require(manifest.get("schema_version") == 8 and manifest.get("status") == "passed", f"seed{seed} manifest failed")
        ...
        for cell in CELLS:
            ...
            clean_aux_routers = item.get("r27_clean_aux_routers")
            if cell in "ab":
...
def validate_formal_lineage(protocol: dict[str, Any], protocol_path: Path) -> int:
    ...
    for seed in SEEDS:
        ...
        for cell in CELLS:
            ...
            require(
                request.get("runtime", {}).get("python")
                == protocol["runtime_binding"]["interpreter"]["executable"],
                f"formal interpreter drift: {seed}/{cell}",
            )
            ...
            require(
                ...
                == Path(protocol["data"]["formal"]["path"]).resolve(),
            ...
            expected_name = f"{cell}_formal_seed{seed}_15ep"
...
def main() -> None:
    ...
    protocol = read_json(protocol_path, "r27 protocol")
    assert_protocol_runtime(protocol)
    protocol_sha = sha256(protocol_path)
    require(protocol.get("schema_version") == 8 and protocol.get("experiment_tag") == "r27", "not r27 schema8")
    ...
    require(migration.get("r23_semantics_verified") is True, "r23 model/routing semantics were not verified")
    require(
        migration.get("r23_initializers_or_trained_weights_used_for_training") is False,
        "r27 must not reuse r23/r24/r25 initializer or trained weights",
    )
    ...
    report = {
        ...
        "dead_experts_reported_as_nonblocking_a1_diagnostic": True,
        "formal_request_lineage_verified": True,
        "gpu_schedule_verified": True,
        "shared_gpu_execution_authorized_and_verified": True,
