#!/usr/bin/env python3
"""Run immutable r25 stage waves on two GPUs, blocked by seed.

Each seed observes A -> B -> C -> D on one physical GPU (C -> D for routing
probes).  Seeds 260829 and 260831 are pinned to GPU0 and seed 260830 to GPU1.
Requests continue to see logical device 0 through CUDA_VISIBLE_DEVICES.
"""
...
from p1_r25_integrity import verify_registered_data_content

SEEDS = (260829, 260830, 260831)
SEED_GPU = {260829: "0", 260830: "1", 260831: "0"}
...
def stage_schedule(stage: str) -> tuple[tuple[tuple[int, str, str], ...], ...]:
    cells = "cd" if stage == "routing_probe" else "abcd"
    waves: list[tuple[tuple[int, str, str], ...]] = []
    # First execute two independent seed blocks concurrently, synchronized at
    # cell boundaries.  Then execute the second GPU0 seed block.
    for cell in cells:
        waves.append(((260829, cell, "0"), (260830, cell, "1")))
    for cell in cells:
        waves.append(((260831, cell, "0"),))
    return tuple(waves)
...
def validate_protocol(protocol: dict[str, Any], protocol_path: Path) -> None:
    require(protocol.get("schema_version") == 8, "r25 runner requires protocol schema 8")
    require(protocol.get("experiment_tag") == "r25", "runner protocol is not r25")
    require(tuple(protocol.get("seeds", ())) == SEEDS, "r25 seed registry drift")
    require(protocol.get("request_count") == 30, "r25 protocol must register exactly 30 requests")
    run_root = Path(protocol["run_root"])
    require(run_root.name == "p1_factorial_full_r25", f"unexpected run root: {run_root}")
    require("r19" not in normalized(run_root), f"run root references r19: {run_root}")
    expected_gpu = {str(seed): gpu for seed, gpu in SEED_GPU.items()}
    require(
        protocol.get("gpu_policy", {}).get("seed_to_physical_gpu") == expected_gpu,
        "protocol seed/GPU mapping drift",
    )
    require(
        ...
        == {
            ...
            "minimum_free_memory_mib_per_gpu_at_stage_start": MIN_FREE_MEMORY_MIB,
    ...
    for stage, expected_cells in (("preflight", set("abcd")), ("routing_probe", set("cd")), ("formal", set("abcd"))):
        ...
        for seed in SEEDS:
            ...
            for cell, entry in seed_map[str(seed)].items():
                ...
                require(protocol_path.parent in request_path.resolve().parents, f"request is outside r25 config: {request_path}")
...
def locked_runner(protocol: dict[str, Any], repo: Path, override: Path | None) -> tuple[Path, str]:
    ...
    if override is not None:
        require(override.resolve() == expected, f"--runner may only name the locked r25 runner: {expected}")
...
def validate_runtime_repository(protocol: dict[str, Any], repo: Path) -> None:
    ...
    require(not dirty, "r25 worktree must be clean before starting a stage")
...
def validate_child_interpreter(protocol: dict[str, Any], repo: Path, requests: list[dict]) -> dict:
    """Prove the exact registered child Python can import torch and r25."""
    ...
    runtime_script = repo / "scripts/a1/p1_r25_runtime.py"
...
def initializer_attestation(protocol: dict[str, Any], seed: int, cell: str) -> dict[str, Any]:
    ...
    routers = cell_payload.get("r25_clean_aux_routers", [])
...
def request_entry(
    ...
    expected_epochs = 30 if stage == "formal" else 1
    ...
    require(
        bool(attestation["cell"].get("r25_clean_aux_routers")) is (cell in "cd"),
...
def main() -> None:
    ...
    stage_requests = []
    for wave in stage_schedule(args.stage):
        for seed, cell, _ in wave:
    ...
    if stage_root.exists():
        raise FileExistsError(f"refusing to adopt or reuse r25 stage directory: {stage_root}")
    inventory = gpu_inventory()
    require(set(SEED_GPU.values()) <= set(inventory), "required physical GPU0/GPU1 are unavailable")
    observed_compute_processes = gpu_compute_processes()
    target_uuids = {inventory[index]["uuid"] for index in set(SEED_GPU.values())}
    preexisting_compute_processes = [
        process for process in observed_compute_processes if process["gpu_uuid"] in target_uuids
    ]
    for index in set(SEED_GPU.values()):
        require(
            inventory[index]["memory_free_mib"] >= MIN_FREE_MEMORY_MIB,
            f"GPU{index} has only {inventory[index]['memory_free_mib']} MiB free; requires {MIN_FREE_MEMORY_MIB}",
        )
    status: dict[str, Any] = {
        "schema_version": 8,
        "stage": args.stage,
        ...
        "seed_to_physical_gpu": {str(seed): gpu for seed, gpu in SEED_GPU.items()},
        "within_seed_order": list("cd" if args.stage == "routing_probe" else "abcd"),
        "waves": [],
    }
    write_json(status_path, status)
    for wave_index, assignments in enumerate(stage_schedule(args.stage), start=1):
        ...
        for seed, cell, physical_gpu in assignments:
            require(SEED_GPU[seed] == physical_gpu, f"{seed}/{cell}: physical GPU mapping drift")
