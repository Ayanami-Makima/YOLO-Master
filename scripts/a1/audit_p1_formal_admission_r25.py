#!/usr/bin/env python3
"""Compose the fail-closed r25 formal-admission evidence."""
...
def validate_implementation(protocol: dict[str, Any]) -> None:
    ...
    require(not dirty, "r25 worktree is dirty at formal admission")
...
def validate_stage_execution(protocol: dict[str, Any], protocol_path: Path, stage: str, cells: str) -> dict[str, str]:
    ...
    runtime_relative = "scripts/a1/p1_r25_runtime.py"
    require(
    ...
    require(
        shared_policy.get("minimum_free_memory_mib_per_gpu_at_stage_start") == 12000,
        f"{stage} shared-GPU memory threshold drift",
    )
    ...
    require(
        all(inventory[index].get("memory_free_mib", 0) >= 12000 for index in ("0", "1")),
        f"{stage} started below the 12GB free-memory threshold",
    )
    require(
...
def validate_initializers(protocol: dict[str, Any], protocol_path: Path) -> tuple[list[dict[str, str]], bool]:
    ...
    for seed in SEEDS:
        ...
        for cell in CELLS:
            ...
            clean_aux_routers = item.get("r25_clean_aux_routers")
            if cell in "ab":
...
def validate_formal_lineage(protocol: dict[str, Any], protocol_path: Path) -> int:
    ...
    for seed in SEEDS:
        ...
        for cell in CELLS:
            ...
            expected_name = f"{cell}_formal_seed{seed}_30ep"
...
def main() -> None:
    ...
    protocol = read_json(protocol_path, "r25 protocol")
    assert_protocol_runtime(protocol)
    protocol_sha = sha256(protocol_path)
    require(protocol.get("schema_version") == 8 and protocol.get("experiment_tag") == "r25", "not r25 schema8")
    ...
    require(
        ...
        "r25 must not reuse r23 initializer or trained weights",
    )
