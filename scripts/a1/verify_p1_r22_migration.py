#!/usr/bin/env python3
"""Verify r22 implementation identity, A/B invariance, and C/D policy parity."""
...
from run_p1_bn_frozen_r22 import R22_CLEAN_AUX_POLICY
...
def verify_git_lineage(repo: Path, protocol: dict[str, Any]) -> dict[str, str]:
    ...
    require(not dirty, "r22 worktree is dirty during migration audit")
...
def main() -> None:
    ...
    for seed in SEEDS:
        ...
        for cell in "ab":
            r22_path = Path(protocol["run_root"]) / "initializers" / f"seed{seed}" / f"{cell}_residual_factor_init.pt"
            r19_path = Path(r19["run_root"]) / "initializers" / f"seed{seed}" / f"{cell}_residual_factor_init.pt"
            require(r22_path.is_file() and r19_path.is_file(), f"missing A/B migration inputs: {seed}/{cell}")
            r22_model, _ = load_checkpoint(r22_path, device="cpu")
            r19_model, _ = load_checkpoint(r19_path, device="cpu")
            r22_model, r19_model = r22_model.float().eval(), r19_model.float().eval()
            tensors_equal, differing = state_equal(r22_model, r19_model)
            output_report = equivalence_report(r19_model, r22_model, sample)
            passed = tensors_equal and output_report.get("max_abs_error") == 0.0
            require(passed, f"r22 A/B invariance failed: {seed}/{cell}")
            comparisons[str(seed)][cell] = {
                "r22": str(r22_path),
                "r22_sha256": sha256(r22_path),
        ...
        for cell in "cd":
            ...
            require(
                all(item[3] and item[4] == R22_CLEAN_AUX_POLICY["runtime_semantics"] for item in signatures[cell]),
