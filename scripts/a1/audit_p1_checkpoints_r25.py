#!/usr/bin/env python3
"""Audit every seed/cell r25 checkpoint against its original initializer."""
...
from run_p1_bn_frozen_r25 import (
    R25_CLEAN_AUX_POLICY,
    R25_DENSE_AUX_POLICY,
    R25_GAIN_POLICY,
    R25_ROUTING_SEMANTICS,
)
...
def runtime_policy(run_dir: Path, *, routed_cell: bool, stage: str, seed: int) -> dict:
    """Verify the persisted r25 freeze, optimizer, and routing policy."""
    path = run_dir / "p1_runtime_policy.json"
    payload = json.loads(path.read_text(encoding="utf-8")) if path.is_file() else {}
    reasons = []
    if payload.get("routing_semantics") != R25_ROUTING_SEMANTICS:
        reasons.append("routing semantics mismatch")
    expected_aux = R25_CLEAN_AUX_POLICY if routed_cell else R25_DENSE_AUX_POLICY
    ...
    # Ultralytics uses ceil(images / physical_batch) microbatches per epoch.
    # Full COCO train2017 has 118287 images: ceil(118287 / 4) * 30 = 887160.
    expected_microbatches = {"preflight": 64, "routing_probe": 1250, "formal": 887160}[stage]
    ...
    for index, module in enumerate(routed):
        ...
        if module.get("routing_aux_semantics") != R25_CLEAN_AUX_POLICY["runtime_semantics"]:
    ...
    if (
        len(gain_groups) != 1
        or not math.isclose(float(gain_groups[0].get("initial_lr", -1)), R25_GAIN_POLICY["lr"], abs_tol=1e-12)
...
def main() -> None:
    ...
    if protocol.get("schema_version") != 8 or protocol.get("experiment_tag") != "r25":
        raise ValueError("r25 checkpoint audit requires an r25 schema-8 protocol")
