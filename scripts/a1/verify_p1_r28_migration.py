#!/usr/bin/env python3
"""Verify r28 implementation identity, A/B invariance, and C/D policy parity."""
...
def verify_git_lineage(repo: Path, protocol: dict[str, Any]) -> dict[str, str]:
    ...
    require(not dirty, "r28 worktree is dirty during migration audit")
...
def main() -> None:
    ...
    # r28 deliberately recalibrates factor BatchNorm on the first 512 medium-COCO
    ...
    for key in semantic_keys:
        ...
        require(equal, f"r28 semantic drift from r23: {key}")
    ...
    require(
        ...
        and co_location.get("maximum_jobs_per_gpu") == 1,
        "single-job-per-GPU authorization or threshold drift",
    )
    for cell in "abcd":
        current = protocol["configs"][cell]
        previous = r23["configs"][cell]
        for key in ("moe", "end2end", "sha256"):
            require(current.get(key) == previous.get(key), f"r28 config semantic drift: {cell}/{key}")
    for key in ("images", "batch", "imgsz", "augment", "shuffle", "grad", "post_calibration_training_policy"):
        require(protocol["batch_norm"].get(key) == r23["batch_norm"].get(key), f"r28 BN policy drift: {key}")
    ...
    for seed in SEEDS:
        ...
        for cell in "ab":
            r28_path = Path(protocol["run_root"]) / "initializers" / f"seed{seed}" / f"{cell}_residual_factor_init.pt"
            require(r28_path.is_file(), f"missing A/B initializer: {seed}/{cell}")
            ...
            require(passed, f"r28 A/B invariance failed: {seed}/{cell}")
            comparisons[str(seed)][cell] = {
                "r28": str(r28_path),
                "r28_sha256": sha256(r28_path),
        ...
        for cell in "cd":
            ...
            require(
                all(item[3] and item[4] == R28_CLEAN_AUX_POLICY["runtime_semantics"] for item in signatures[cell]),
