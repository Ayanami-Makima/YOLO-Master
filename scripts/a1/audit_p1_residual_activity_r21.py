#!/usr/bin/env python3
"""Audit r21 residual-factor activity on the fixed pilot-validation sample.

This diagnostic treats every routing-probe checkpoint as immutable. It evaluates
the exact C/D ``last.pt`` files registered by the r21 protocol and measures, for
layers 4, 6, and 8,
...
from run_p1_bn_frozen_r21 import R21_CLEAN_AUX_POLICY
...
def validate_protocol(protocol_path: Path, protocol: dict[str, Any]) -> tuple[list[int], Path]:
    """Validate r21 identity, hash-locked inputs, and C/D probe requests."""
    require(protocol.get("schema_version") == EXPECTED_PROTOCOL_SCHEMA, "protocol schema is not r21 schema 7")
    require(protocol.get("experiment_tag") == "r21", "protocol experiment_tag is not r21")
    require(protocol.get("primary_checkpoint") == "last.pt", "r21 primary checkpoint must be last.pt")
    ...
    require(repo.is_dir(), f"registered r21 repository is missing: {repo}")
...
def validate_checkpoint_audit(
    ...
    require(audit.get("schema_version") == EXPECTED_PROTOCOL_SCHEMA, "checkpoint-audit schema is not 7")
...
def validate_routing_audit(
    ...
    require(routing.get("schema_version") == EXPECTED_PROTOCOL_SCHEMA, "routing-audit schema is not 7")
...
def disable_routing_noise(model: torch.nn.Module) -> list[dict[str, Any]]:
    ...
    for name, module in model.named_modules():
        ...
        require(
            getattr(module, "routing_aux_semantics", None) == R21_CLEAN_AUX_POLICY["runtime_semantics"],
