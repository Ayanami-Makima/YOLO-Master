#!/usr/bin/env python3
"""Run immutable r23 stage waves on two GPUs, blocked by seed.
...
def validate_protocol(protocol: dict[str, Any], protocol_path: Path) -> None:
    require(protocol.get("schema_version") == 8, "r23 runner requires protocol schema 8")
    require(protocol.get("experiment_tag") == "r23", "runner protocol is not r23")
    require(tuple(protocol.get("seeds", ())) == SEEDS, "r23 seed registry drift")
    require(protocol.get("request_count") == 30, "r23 protocol must register exactly 30 requests")
    run_root = Path(protocol["run_root"])
    require(run_root.name == "p1_factorial_r23", f"unexpected run root: {run_root}")
    ...
    for stage, expected_cells in (("preflight", set("abcd")), ("routing_probe", set("cd")), ("formal", set("abcd"))):
        ...
        for seed in SEEDS:
            ...
            for cell, entry in seed_map[str(seed)].items():
                ...
                require(protocol_path.parent in request_path.resolve().parents, f"request is outside r23 config: {request_path}")
...
def locked_runner(protocol: dict[str, Any], repo: Path, override: Path | None) -> tuple[Path, str]:
    ...
    if override is not None:
        require(override.resolve() == expected, f"--runner may only name the locked r23 runner: {expected}")
...
def validate_runtime_repository(protocol: dict[str, Any], repo: Path) -> None:
    ...
    require(not dirty, "r23 worktree must be clean before starting a stage")
...
def validate_child_interpreter(protocol: dict[str, Any], repo: Path, requests: list[dict]) -> dict:
    """Prove the exact registered child Python can import torch and r23."""
    ...
    runtime_script = repo / "scripts/a1/p1_r23_runtime.py"
...
def initializer_attestation(protocol: dict[str, Any], seed: int, cell: str) -> dict[str, Any]:
    ...
    routers = cell_payload.get("r23_clean_aux_routers", [])
...
def request_entry(
    ...
    require(
        bool(attestation["cell"].get("r23_clean_aux_routers")) is (cell in "cd"),
...
def main() -> None:
    ...
    if stage_root.exists():
        raise FileExistsError(f"refusing to adopt or reuse r23 stage directory: {stage_root}")
