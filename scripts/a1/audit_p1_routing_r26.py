#!/usr/bin/env python3
"""Apply the A1-aligned fixed-512 hard Top-2 diagnostics to r26 C/D probes.
    ...
    for name, module in model.model.named_modules():
        ...
        if routing is not None and hasattr(routing, "num_experts") and hasattr(routing, "top_k"):
            if getattr(routing, "p1_balance_on_clean_routes", False) is not True:
                raise ValueError(f"r26 clean-aux flag missing: {name}.routing")
            if getattr(module, "routing_aux_semantics", None) != R26_CLEAN_AUX_POLICY["runtime_semantics"]:
                raise ValueError(f"r26 routing-aux semantics mismatch: {name}.routing")
...
def main() -> None:
    ...
    if protocol.get("schema_version") != 8 or protocol.get("experiment_tag") != "r26":
        raise ValueError("r26 routing audit requires an r26 schema-8 protocol")
    ...
    if output.exists():
        raise FileExistsError("refusing to overwrite r26 routing evidence")
