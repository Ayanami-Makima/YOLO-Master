#!/usr/bin/env python3
"""Audit r28 residual-factor activity on an independent fixed pilot-val512 sample.

This diagnostic treats every routing-probe checkpoint as immutable. It evaluates
the exact C/D ``last.pt`` files registered by the r28 protocol and measures, for
layers 4, 6, and 8,
...
from run_p1_bn_frozen_r28 import R28_CLEAN_AUX_POLICY
...
def validate_protocol(protocol_path: Path, protocol: dict[str, Any]) -> tuple[list[int], Path]:
    """Validate r28 identity, hash-locked inputs, and C/D probe requests."""
    require(protocol.get("schema_version") == EXPECTED_PROTOCOL_SCHEMA, "protocol schema is not r28 schema 8")
    require(protocol.get("experiment_tag") == "r28", "protocol experiment_tag is not r28")
    require(protocol.get("primary_checkpoint") == "last.pt", "r28 primary checkpoint must be last.pt")
    ...
    require(repo.is_dir(), f"registered r28 repository is missing: {repo}")
...
def disable_routing_noise(model: torch.nn.Module) -> list[dict[str, Any]]:
    ...
    for name, module in model.named_modules():
        ...
        require(
            getattr(module, "routing_aux_semantics", None) == R28_CLEAN_AUX_POLICY["runtime_semantics"],
