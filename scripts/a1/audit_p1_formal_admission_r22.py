#!/usr/bin/env python3
"""Compose the fail-closed r22 formal-admission evidence."""
...
def validate_implementation(protocol: dict[str, Any]) -> None:
    ...
    require(not dirty, "r22 worktree is dirty at formal admission")
...
def validate_stage_execution(protocol: dict[str, Any], protocol_path: Path, stage: str, cells: str) -> dict[str, str]:
    ...
    runtime_relative = "scripts/a1/p1_r22_runtime.py"
    require(
...
def validate_initializers(protocol: dict[str, Any], protocol_path: Path) -> tuple[list[dict[str, str]], bool]:
    ...
    for seed in SEEDS:
        ...
        for cell in CELLS:
            ...
            clean_aux_routers = item.get("r22_clean_aux_routers")
            if cell in "ab":
...
def main() -> None:
    ...
    protocol = read_json(protocol_path, "r22 protocol")
    assert_protocol_runtime(protocol)
    protocol_sha = sha256(protocol_path)
    require(protocol.get("schema_version") == 8 and protocol.get("experiment_tag") == "r22", "not r22 schema8")
