...
PROTOCOL="$REPO/configs/a1/p1_factorial_medium_r28/protocol.json"
RUN_ROOT=/data/data2/TuJiajun/A1-smoke-r4/p1_factorial_medium_r28
...
echo "$(timestamp) r28_pipeline_started"

wait_for_gpu_memory preflight
"$PYTHON" scripts/a1/run_p1_factorial_multiseed_2gpu_r28.py \
    --protocol "$PROTOCOL" --stage preflight
"$PYTHON" scripts/a1/audit_p1_checkpoints_r28.py \
    --protocol "$PROTOCOL" --stage preflight \
...
env CUDA_VISIBLE_DEVICES=0 "$PYTHON" scripts/a1/audit_p1_routing_r28.py \
    --protocol "$PROTOCOL" \
    --preflight-audit "$AUDITS/preflight_checkpoints.json" \
    --probe-audit "$AUDITS/routing_probe_checkpoints.json" \
    --device 0
env CUDA_VISIBLE_DEVICES=0 "$PYTHON" scripts/a1/audit_p1_residual_activity_r28.py \
    --protocol "$PROTOCOL" \
...
"$PYTHON" scripts/a1/audit_p1_formal_admission_r28.py \
    --protocol "$PROTOCOL" \
    --preflight-audit "$AUDITS/preflight_checkpoints.json" \
    --probe-audit "$AUDITS/routing_probe_checkpoints.json" \
    --routing-audit "$AUDITS/routing/hard_top2_5000.json" \
    --residual-audit "$AUDITS/residual_activity/routing_probe_512.json" \
    --migration-audit "$AUDITS/migration_r23_to_r28.json" \
...
echo "$(timestamp) r28_pipeline_completed"
