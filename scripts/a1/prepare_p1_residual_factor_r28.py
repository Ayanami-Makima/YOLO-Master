#!/usr/bin/env python3
"""Generate the immutable A1 P1 medium-COCO r28 residual-factor protocol.

r28 preserves r23's model, routing, initializer, freeze, runtime-source binding,
and admission semantics.  The only experimental changes are the formal dataset
(a deterministic 20,000-image train subset and full 5,000-image val set) and the
formal duration (15 epochs).  Preflight
...
import yaml
from p1_r28_runtime import RUNTIME_ATTESTATION

# isort: split

from p1_r28_integrity import data_list_content_signature
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--experiment-tag", default="r28")
...
def require_locked_baseline_ancestor(repo: Path) -> None:
    """Fail unless the r28 implementation descends from the locked official baseline."""
...
def validate_destinations(repo: Path, run_root: Path, output: Path, tag: str) -> None:
    if tag != "r28":
        raise ValueError(f"r28 protocol requires experiment-tag r28, got {tag!r}")
    if repo.name != "YOLO-Master-r28-medium":
        raise ValueError(f"r28 requires isolated worktree YOLO-Master-r28-medium, got {repo}")
    expected_output = repo / "configs/a1/p1_factorial_medium_r28"
    if not same_path(output, expected_output):
        raise ValueError(f"r28 config directory must be {expected_output}, got {output}")
    if run_root.name != "p1_factorial_medium_r28":
        raise ValueError(f"r28 run root must end in p1_factorial_medium_r28, got {run_root}")
...
def calibration_images(formal_data: Path) -> tuple[list[str], Path]:
    ...
    if not isinstance(train, str):
        raise TypeError("r28 requires formal train to be one explicit image-list file")
    ...
    if len(lines) < 512:
        raise ValueError(f"r28 BN calibration needs 512 train images, found {len(lines)}")
    resolved: list[str] = []
...
def data_registry_entry(
    ...
    for split, expected_count in (("train", expected_train), ("val", expected_val)):
        ...
        if not isinstance(entry, str):
            raise TypeError(f"r28 requires {data_yaml} {split} to be one explicit image-list file")
...
def main() -> None:
    ...
    if python != EXPECTED_PYTHON:
        raise ValueError(f"r28 requires the exact virtualenv launcher {EXPECTED_PYTHON}, got {python}")
    if RUNTIME_ATTESTATION["interpreter"]["executable"] != str(python):
        raise RuntimeError("r28 protocol generator was not launched by the registered virtualenv interpreter")
    if RUNTIME_ATTESTATION["interpreter"]["virtualenv"] is not True:
        raise RuntimeError("r28 protocol generation requires an active virtualenv interpreter")
    forbid_r19_artifact(checkpoint, "source checkpoint")
    if checkpoint != OFFICIAL_CHECKPOINT.resolve():
        raise ValueError(f"r28 must start from the locked official checkpoint {OFFICIAL_CHECKPOINT}, got {checkpoint}")
    if sha256(checkpoint) != OFFICIAL_CHECKPOINT_SHA256:
        raise ValueError("official yolo26n.pt SHA-256 mismatch")
    branch = git_output(repo, "branch", "--show-current")
    if branch != "a1-p1-medium-r28":
        raise ValueError(f"r28 worktree branch must be a1-p1-medium-r28, got {branch!r}")
    require_locked_baseline_ancestor(repo)
    dirty = git_output(repo, "status", "--porcelain")
    if dirty:
        raise ValueError("implementation worktree must be clean before generating the r28 protocol")
    ...
    implementation_paths = (
        ...
        *moe_runtime_paths,
        ...
        repo / "scripts/a1/prepare_p1_residual_factor_r28.py",
        repo / "scripts/a1/prepare_p1_medium_data_r28.py",
        repo / "scripts/a1/p1_r28_integrity.py",
        repo / "scripts/a1/p1_r28_runtime.py",
        repo / "scripts/a1/prepare_p1_factorial.py",
        repo / "scripts/a1/build_p1_residual_factor_initializers_r28.py",
        repo / "scripts/a1/build_p1_residual_factor_initializers.py",
        repo / "scripts/a1/run_p1_bn_frozen_r28.py",
        repo / "scripts/a1/run_p1_factorial_multiseed_2gpu_r28.py",
        repo / "scripts/a1/audit_p1_checkpoints_r28.py",
        repo / "scripts/a1/audit_p1_residual_factor_checkpoints.py",
        repo / "scripts/a1/audit_p1_routing_r28.py",
        repo / "scripts/a1/audit_p1_preflight_routing.py",
        repo / "scripts/a1/audit_p1_routing.py",
        repo / "scripts/a1/evaluate_p1_matrix.py",
        repo / "scripts/a1/audit_p1_residual_activity_r28.py",
        repo / "scripts/a1/audit_p1_formal_admission_r28.py",
        repo / "scripts/a1/verify_p1_r28_migration.py",
        repo / "tests/test_p1_residual_factor.py",
        repo / "tests/test_moe_router_boundaries.py",
        repo / "tests/test_peft_optimizer_policy.py",
        repo / "tests/test_p1_r28_protocol.py",
    )
    ...
    formal_registry = data_registry_entry(formal_data, expected_train=20000, expected_val=5000)
    ...
    for seed in SEEDS:
        ...
        for cell in CELLS:
            ...
            for stage, data, epochs in (("preflight", preflight_data, 1), ("formal", formal_data, 15)):
    ...
    protocol: dict[str, Any] = {
        "schema_version": 8,
        "name": "A1 P1 medium-COCO equal-budget 2x2 factorial r28 residual-factor",
        "experiment_tag": "r28",
        ...
        "factor_gain_optimizer": GAIN_POLICY,
        "batch_norm": {
            "calibration_split": "formal deterministic COCO train subset only",
            "selection": "first 512 paths in the immutable medium-COCO train list",
        ...
        "r28_medium_coco_extension": {
            "scope": "medium-COCO formal budget plus user-authorized shared-GPU execution; authorized before any r28 training",
            "change": "replace the 5000/512 five-epoch formal pilot with deterministic COCO train20k/val5k for 15 epochs",
            "authorization": "user explicitly requested 20000/5000 and 15-epoch P1 on 2026-08-30",
            "r23_initializers_or_trained_weights_reused": False,
            "r24_initializers_or_trained_weights_reused": False,
            "r25_initializers_or_trained_weights_reused": False,
            "r26_initializers_or_trained_weights_reused": False,
            "r27_initializers_or_trained_weights_reused": False,
            ...
            "budget_changed": True,
            "formal_train_images": 20000,
            "formal_validation_images": 5000,
            "formal_epochs": 15,
            "formal_subset_selection": "Python random.Random(260829).sample without replacement from ordered full train2017 list",
            "formal_data_integrity": "YAML SHA256 plus ordered train/val list SHA256, exact counts, and image/label content SHA256",
            "per_file_content_hashing_required": True,
            "inference_changed": False,
        },
        "r28_routing_audit_correction": {
            "trigger": "r26 fixed-val512 observed 410/512 selections in one router against a strict 0.8 cap",
            "change": "retain the exact 0.8 and 0.5 thresholds but evaluate every router on full formal val5000",
            "fixed_val512_result_reused": False,
            "r26_probe_or_initializer_weights_reused": False,
            "model_training_hyperparameters_changed_from_r26": False,
            "authorization": "user approved the full-val5000 correction on 2026-08-30",
        },
        "r28_single_job_gpu_correction": {
            "trigger": "r27 routing probe OOM when C and D were co-located on each GPU",
            "change": "cap every stage at one independent experiment process per physical GPU",
            "minimum_free_memory_mib_to_start_one_job": 12000,
            "r27_probe_or_initializer_weights_reused": False,
            "model_training_hyperparameters_changed_from_r27": False,
            "authorization": "user explicitly approved at most one job per GPU on 2026-08-30",
        },
        "routing_gate": {
            "sample": "complete registered formal val 5000",
            "dead_experts": {
                "target": 0,
                "blocking": False,
        ...
        "routing_gate_scope": "A1-aligned collapse admission: entropy and concentration remain blocking on full val5000; dead-expert coverage is reported but non-blocking",
        "residual_activity_gate": {
            "checkpoint": "routing-probe last.pt",
            "sample": "independent fixed pilot val512; checkpoint identity is shared with the full-val5000 routing audit",
        ...
        "routing_probe_policy": "C/D x 3 seeds start independently from original r28 initializers; discard all weights",
        ...
        "gpu_policy": {
            "mode": "deterministic independent jobs on GPU0/GPU1 with at most one job per GPU; never DDP; request-visible device is logical 0",
            "ordered_job_queue": "seed ascending, then A/B/C/D; routing probes seed ascending then C/D",
            "assignment": "each wave assigns at most one queued job to each GPU whose stage-start free memory is at least 12000 MiB",
            "co_location": {
                "authorized": True,
                "authorization": "user explicitly required at most one job per GPU on 2026-08-30",
                "one_job_minimum_free_memory_mib": 12000,
        ...
        "formal_admission": {
            ...
            "must_bind": [
                ...
                "full_val5000_hard_top2_collapse_gate_36_of_36_with_dead_expert_diagnostic",
