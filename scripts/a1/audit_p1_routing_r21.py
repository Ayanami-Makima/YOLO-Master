#!/usr/bin/env python3
"""Apply the immutable fixed-512 hard Top-2 gate to all r21 C/D probes.
    ...
    for name, module in model.model.named_modules():
        ...
        if routing is not None and hasattr(routing, "num_experts") and hasattr(routing, "top_k"):
            if getattr(routing, "p1_balance_on_clean_routes", False) is not True:
                raise ValueError(f"r21 clean-aux flag missing: {name}.routing")
            if getattr(module, "routing_aux_semantics", None) != R21_CLEAN_AUX_POLICY["runtime_semantics"]:
                raise ValueError(f"r21 routing-aux semantics mismatch: {name}.routing")
...
def main() -> None:
    ...
    if protocol.get("schema_version") != 7 or protocol.get("experiment_tag") != "r21":
        raise ValueError("r21 routing audit requires an r21 schema-7 protocol")
    ...
    if output.exists():
        raise FileExistsError("refusing to overwrite r21 routing evidence")
