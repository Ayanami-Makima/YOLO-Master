#!/usr/bin/env python3
"""Verify r20 implementation identity, A/B invariance, and C/D policy parity."""
...
def verify_git_lineage(repo: Path, protocol: dict[str, Any]) -> dict[str, str]:
    ...
    require(not dirty, "r20 worktree is dirty during migration audit")
...
def main() -> None:
    ...
    require(protocol.get("schema_version") == 6 and protocol.get("experiment_tag") == "r20", "not r20 schema6")
    ...
    for seed in SEEDS:
        ...
        for cell in "ab":
            r20_path = Path(protocol["run_root"]) / "initializers" / f"seed{seed}" / f"{cell}_residual_factor_init.pt"
            r19_path = Path(r19["run_root"]) / "initializers" / f"seed{seed}" / f"{cell}_residual_factor_init.pt"
            require(r20_path.is_file() and r19_path.is_file(), f"missing A/B migration inputs: {seed}/{cell}")
            r20_model, _ = load_checkpoint(r20_path, device="cpu")
            r19_model, _ = load_checkpoint(r19_path, device="cpu")
            r20_model, r19_model = r20_model.float().eval(), r19_model.float().eval()
            tensors_equal, differing = state_equal(r20_model, r19_model)
            output_report = equivalence_report(r19_model, r20_model, sample)
            passed = tensors_equal and output_report.get("max_abs_error") == 0.0
            require(passed, f"r20 A/B invariance failed: {seed}/{cell}")
            comparisons[str(seed)][cell] = {
                "r20": str(r20_path),
                "r20_sha256": sha256(r20_path),
        ...
        for cell in "cd":
            ...
            require(
                all(item[3] and item[4] == R20_CLEAN_AUX_POLICY["runtime_semantics"] for item in signatures[cell]),
