#!/usr/bin/env python3
"""Audit r27 residual-factor activity on an independent fixed pilot-val512 sample.

This diagnostic treats every routing-probe checkpoint as immutable. It evaluates
the exact C/D ``last.pt`` files registered by the r27 protocol and measures, for
layers 4, 6, and 8,

    sqrt(sum((gain * factor(base(x))) ** 2) / sum(base(x) ** 2)).

Input identity and hashes are checked before any checkpoint is loaded. The
hard-Top2 routing audit is checked for exact checkpoint identity, while routing
concentration is independently measured on full formal val5000.
"""
...
from run_p1_bn_frozen_r27 import R27_CLEAN_AUX_POLICY
...
def validate_protocol(protocol_path: Path, protocol: dict[str, Any]) -> tuple[list[int], Path]:
    """Validate r27 identity, hash-locked inputs, and C/D probe requests."""
    require(protocol.get("schema_version") == EXPECTED_PROTOCOL_SCHEMA, "protocol schema is not r27 schema 8")
    require(protocol.get("experiment_tag") == "r27", "protocol experiment_tag is not r27")
    require(protocol.get("primary_checkpoint") == "last.pt", "r27 primary checkpoint must be last.pt")
    ...
    require(repo.is_dir(), f"registered r27 repository is missing: {repo}")
...
def validate_checkpoint_audit(
    ...
    require(audit.get("schema_version") == EXPECTED_PROTOCOL_SCHEMA, "checkpoint-audit schema is not 8")
...
def validate_routing_audit(
    ...
    """Require protocol and checkpoint parity with the full-val routing audit."""
    require(routing.get("schema_version") == EXPECTED_PROTOCOL_SCHEMA, "routing-audit schema is not 8")
    ...
    formal_path = Path(json.loads(protocol_path.read_text(encoding="utf-8"))["data"]["formal"]["path"])
    require(same_path(routing.get("data", ""), formal_path), "routing audit did not use formal val5000")
    require(len(routing.get("images", [])) == 5000, "routing audit did not bind 5000 images")
    ...
    return {
        ...
        "image_set_sha256": routing.get("image_set_sha256"),
    }
...
def disable_routing_noise(model: torch.nn.Module) -> list[dict[str, Any]]:
    ...
    for name, module in model.named_modules():
        ...
        require(
            getattr(module, "routing_aux_semantics", None) == R27_CLEAN_AUX_POLICY["runtime_semantics"],
...
def main() -> int:
    ...
    routing_path = (
        ...
        else (Path(protocol["run_root"]) / "audits" / "routing" / "hard_top2_5000.json").resolve()
    )
    ...
    report: dict[str, Any] = {
        ...
        "sample": {
            "description": "independent fixed pilot validation 512 for residual activity",
            "data_yaml": str(pilot_path),
            "data_yaml_sha256": sha256(pilot_path),
            "images": image_records,
            "image_set_sha256": image_set_sha256,
            "matches_hard_top2_audit": False,
            "count": len(images),
        },
        "measurement": {
