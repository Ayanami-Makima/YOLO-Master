#!/usr/bin/env python3
"""Apply the immutable fixed-512 hard Top-2 gate to all r22 C/D probes.

This script emits gate evidence only. Formal admission is composed separately
after checkpoint, residual-activity, lineage, and migration audits pass.
"""
    ...
    for name, module in model.model.named_modules():
        ...
        if routing is not None and hasattr(routing, "num_experts") and hasattr(routing, "top_k"):
            if getattr(routing, "p1_balance_on_clean_routes", False) is not True:
                raise ValueError(f"r22 clean-aux flag missing: {name}.routing")
            if getattr(module, "routing_aux_semantics", None) != R22_CLEAN_AUX_POLICY["runtime_semantics"]:
                raise ValueError(f"r22 routing-aux semantics mismatch: {name}.routing")
...
def main() -> None:
    ...
    if protocol.get("schema_version") != 8 or protocol.get("experiment_tag") != "r22":
        raise ValueError("r22 routing audit requires an r22 schema-8 protocol")
    ...
    if output.exists():
        raise FileExistsError("refusing to overwrite r22 routing evidence")
    ...
    report = {
        ...
        "admission_policy": "gate evidence only; combined admission must also bind the residual-activity addendum",
        "seeds": {},
    }
