...
REPO=/data/data2/TuJiajun/A1-smoke-r4/YOLO-Master-r26-medium
PYTHON=/data/data2/TuJiajun/A1-smoke-r4/.venv/bin/python
PROTOCOL="$REPO/configs/a1/p1_factorial_medium_r26/protocol.json"
RUN_ROOT=/data/data2/TuJiajun/A1-smoke-r4/p1_factorial_medium_r26
...
echo "$(timestamp) r26_pipeline_started"

wait_for_gpu_memory preflight
"$PYTHON" scripts/a1/run_p1_factorial_multiseed_2gpu_r26.py \
    --protocol "$PROTOCOL" --stage preflight
"$PYTHON" scripts/a1/audit_p1_checkpoints_r26.py \
    --protocol "$PROTOCOL" --stage preflight \
...
env CUDA_VISIBLE_DEVICES=0 "$PYTHON" scripts/a1/audit_p1_routing_r26.py \
    --protocol "$PROTOCOL" \
    --preflight-audit "$AUDITS/preflight_checkpoints.json" \
    --probe-audit "$AUDITS/routing_probe_checkpoints.json" \
    --device 0
env CUDA_VISIBLE_DEVICES=0 "$PYTHON" scripts/a1/audit_p1_residual_activity_r26.py \
    --protocol "$PROTOCOL" \
...
"$PYTHON" scripts/a1/audit_p1_formal_admission_r26.py \
    --protocol "$PROTOCOL" \
    --preflight-audit "$AUDITS/preflight_checkpoints.json" \
    --probe-audit "$AUDITS/routing_probe_checkpoints.json" \
    --routing-audit "$AUDITS/routing/hard_top2_512.json" \
    --residual-audit "$AUDITS/residual_activity/routing_probe_512.json" \
    --migration-audit "$AUDITS/migration_r23_to_r26.json" \
...
echo "$(timestamp) r26_pipeline_completed"
