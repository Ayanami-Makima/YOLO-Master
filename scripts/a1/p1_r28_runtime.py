#!/usr/bin/env python3
"""Fail-closed runtime binding for the isolated A1 P1 r28 worktree."""
...
def _module_source(module_name: str) -> Path:
    ...
    if not source:
        raise RuntimeError(f"r28 runtime module has no source path: {module_name}")
...
def assert_r28_runtime() -> dict[str, str]:
    ...
    for name, expected_path in expected.items():
        ...
        if actual_path != expected_path:
            ...
            raise RuntimeError(f"r28 runtime repository mismatch: {json.dumps(mismatch, sort_keys=True)}")
        actual[name] = actual_path
    return {name: str(path) for name, path in actual.items()}


RUNTIME_SOURCES = assert_r28_runtime()
...
def assert_protocol_runtime(protocol: dict) -> None:
    ...
    if protocol.get("runtime_binding") != RUNTIME_ATTESTATION:
        raise RuntimeError("r28 protocol/runtime module provenance mismatch")
