#!/usr/bin/env python3
"""Verify r27 implementation identity, A/B invariance, and C/D policy parity."""
...
def verify_git_lineage(repo: Path, protocol: dict[str, Any]) -> dict[str, str]:
    ...
    require(not dirty, "r27 worktree is dirty during migration audit")
...
def main() -> None:
    ...
    # r27 deliberately recalibrates factor BatchNorm on the first 512 medium-COCO
    ...
    for key in semantic_keys:
        ...
        require(equal, f"r27 semantic drift from r23: {key}")
        semantic_comparison[key] = True
    co_location = protocol["gpu_policy"].get("co_location", {})
    require(
        co_location.get("authorized") is True
        and co_location.get("one_job_minimum_free_memory_mib") == 12000
        and co_location.get("two_jobs_minimum_free_memory_mib") == 22000
        and co_location.get("maximum_jobs_per_gpu") == 2,
        "memory-aware shared-GPU authorization or thresholds drift",
    )
    for cell in "abcd":
        current = protocol["configs"][cell]
        previous = r23["configs"][cell]
        for key in ("moe", "end2end", "sha256"):
            require(current.get(key) == previous.get(key), f"r27 config semantic drift: {cell}/{key}")
    for key in ("images", "batch", "imgsz", "augment", "shuffle", "grad", "post_calibration_training_policy"):
        require(protocol["batch_norm"].get(key) == r23["batch_norm"].get(key), f"r27 BN policy drift: {key}")
    ...
    for seed in SEEDS:
        ...
        for cell in "ab":
            r27_path = Path(protocol["run_root"]) / "initializers" / f"seed{seed}" / f"{cell}_residual_factor_init.pt"
            require(r27_path.is_file(), f"missing A/B initializer: {seed}/{cell}")
            ...
            require(passed, f"r27 A/B invariance failed: {seed}/{cell}")
            comparisons[str(seed)][cell] = {
                "r27": str(r27_path),
                "r27_sha256": sha256(r27_path),
                ...
                "reason": "factor BatchNorm is independently calibrated on the registered medium-COCO train512 sample",
                "passed": passed,
            }
        signatures = {}
        for cell in "cd":
            ...
            require(
                all(item[3] and item[4] == R27_CLEAN_AUX_POLICY["runtime_semantics"] for item in signatures[cell]),
