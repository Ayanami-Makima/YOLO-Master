#!/usr/bin/env python3
"""Generate the immutable A1 P1 r23 three-seed residual-factor protocol.

r23 preserves r21's model, routing, runtime-source binding, budgets, and gates.
Its only repair is to preserve the virtualenv interpreter symlink verbatim and
prove that the exact child interpreter can import torch and the locked r23
modules before any stage directory is created.
"""
...
import yaml
from p1_r23_runtime import RUNTIME_ATTESTATION

# isort: split

from p1_r23_integrity import data_list_content_signature
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--experiment-tag", default="r23")
...
def require_locked_baseline_ancestor(repo: Path) -> None:
    """Fail unless the r23 implementation descends from the locked official baseline."""
...
def validate_destinations(repo: Path, run_root: Path, output: Path, tag: str) -> None:
    if tag != "r23":
        raise ValueError(f"r23 protocol requires experiment-tag r23, got {tag!r}")
    if repo.name != "YOLO-Master-r23":
        raise ValueError(f"r23 requires isolated worktree YOLO-Master-r23, got {repo}")
    expected_output = repo / "configs/a1/p1_factorial_r23"
    if not same_path(output, expected_output):
        raise ValueError(f"r23 config directory must be {expected_output}, got {output}")
    if run_root.name != "p1_factorial_r23":
        raise ValueError(f"r23 run root must end in p1_factorial_r23, got {run_root}")
...
def calibration_images(pilot_data: Path) -> tuple[list[str], Path]:
    ...
    if not isinstance(train, str):
        raise TypeError("r23 requires pilot train to be one explicit image-list file")
    ...
    if len(lines) < 512:
        raise ValueError(f"r23 BN calibration needs 512 train images, found {len(lines)}")
    resolved: list[str] = []
...
def data_registry_entry(data_yaml: Path, *, expected_train: int, expected_val: int) -> dict[str, Any]:
    ...
    for split, expected_count in (("train", expected_train), ("val", expected_val)):
        ...
        if not isinstance(entry, str):
            raise TypeError(f"r23 requires {data_yaml} {split} to be one explicit image-list file")
...
def main() -> None:
    ...
    if python != EXPECTED_PYTHON:
        raise ValueError(f"r23 requires the exact virtualenv launcher {EXPECTED_PYTHON}, got {python}")
    if RUNTIME_ATTESTATION["interpreter"]["executable"] != str(python):
        raise RuntimeError("r23 protocol generator was not launched by the registered virtualenv interpreter")
    if RUNTIME_ATTESTATION["interpreter"]["virtualenv"] is not True:
        raise RuntimeError("r23 protocol generation requires an active virtualenv interpreter")
    forbid_r19_artifact(checkpoint, "source checkpoint")
    if checkpoint != OFFICIAL_CHECKPOINT.resolve():
        raise ValueError(f"r23 must start from the locked official checkpoint {OFFICIAL_CHECKPOINT}, got {checkpoint}")
    if sha256(checkpoint) != OFFICIAL_CHECKPOINT_SHA256:
        raise ValueError("official yolo26n.pt SHA-256 mismatch")
    branch = git_output(repo, "branch", "--show-current")
    if branch != "a1-p1-r23":
        raise ValueError(f"r23 worktree branch must be a1-p1-r23, got {branch!r}")
    require_locked_baseline_ancestor(repo)
    dirty = git_output(repo, "status", "--porcelain")
    if dirty:
        raise ValueError("implementation worktree must be clean before generating the r23 protocol")
    ...
    implementation_paths = (
        ...
        *moe_runtime_paths,
        ...
        repo / "scripts/a1/prepare_p1_residual_factor_r23.py",
        repo / "scripts/a1/p1_r23_integrity.py",
        repo / "scripts/a1/p1_r23_runtime.py",
        repo / "scripts/a1/prepare_p1_factorial.py",
        repo / "scripts/a1/build_p1_residual_factor_initializers_r23.py",
        repo / "scripts/a1/build_p1_residual_factor_initializers.py",
        repo / "scripts/a1/run_p1_bn_frozen_r23.py",
        repo / "scripts/a1/run_p1_factorial_multiseed_2gpu_r23.py",
        repo / "scripts/a1/audit_p1_checkpoints_r23.py",
        repo / "scripts/a1/audit_p1_residual_factor_checkpoints.py",
        repo / "scripts/a1/audit_p1_routing_r23.py",
        repo / "scripts/a1/audit_p1_preflight_routing.py",
        repo / "scripts/a1/audit_p1_routing.py",
        repo / "scripts/a1/evaluate_p1_matrix.py",
        repo / "scripts/a1/audit_p1_residual_activity_r23.py",
        repo / "scripts/a1/audit_p1_formal_admission_r23.py",
        repo / "scripts/a1/verify_p1_r23_migration.py",
        repo / "tests/test_p1_residual_factor.py",
        repo / "tests/test_moe_router_boundaries.py",
        repo / "tests/test_peft_optimizer_policy.py",
        repo / "tests/test_p1_r23_protocol.py",
    )
    ...
    protocol: dict[str, Any] = {
        "schema_version": 8,
        "name": "A1 P1 equal-budget 2x2 factorial r23 residual-factor",
        "experiment_tag": "r23",
        ...
        "r23_taskbook_alignment": {
            "scope": "formal-admission semantics only; authorized before any formal training",
            "change": "report zero-selection experts on the fixed val512 sample as diagnostics instead of treating finite-sample coverage as an A1 blocker",
            "authorization": "user explicitly requested taskbook-aligned formal training on 2026-08-30",
            "r22_initializers_or_trained_weights_reused": False,
            "model_or_routing_semantics_changed_from_r22": False,
            "dead_expert_diagnostic_target_changed": False,
            "dead_expert_blocking_semantics_changed": True,
        ...
        "routing_probe_policy": "C/D x 3 seeds start independently from original r23 initializers; discard all weights",
