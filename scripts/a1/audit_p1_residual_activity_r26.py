#!/usr/bin/env python3
"""Audit r26 residual-factor activity on the fixed pilot-validation sample.

This diagnostic treats every routing-probe checkpoint as immutable. It evaluates
the exact C/D ``last.pt`` files registered by the r26 protocol and measures, for
layers 4, 6, and 8,
...
def validate_protocol(protocol_path: Path, protocol: dict[str, Any]) -> tuple[list[int], Path]:
    """Validate r26 identity, hash-locked inputs, and C/D probe requests."""
    require(protocol.get("schema_version") == EXPECTED_PROTOCOL_SCHEMA, "protocol schema is not r26 schema 8")
    require(protocol.get("experiment_tag") == "r26", "protocol experiment_tag is not r26")
    require(protocol.get("primary_checkpoint") == "last.pt", "r26 primary checkpoint must be last.pt")
    ...
    require(repo.is_dir(), f"registered r26 repository is missing: {repo}")
...
def disable_routing_noise(model: torch.nn.Module) -> list[dict[str, Any]]:
    ...
    for name, module in model.named_modules():
        ...
        require(
            getattr(module, "routing_aux_semantics", None) == R26_CLEAN_AUX_POLICY["runtime_semantics"],
