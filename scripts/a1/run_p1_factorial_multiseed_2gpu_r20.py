#!/usr/bin/env python3
"""Run immutable r20 stage waves on two GPUs, blocked by seed.
...
def validate_protocol(protocol: dict[str, Any], protocol_path: Path) -> None:
    require(protocol.get("schema_version") == 6, "r20 runner requires protocol schema 6")
    require(protocol.get("experiment_tag") == "r20", "runner protocol is not r20")
    require(tuple(protocol.get("seeds", ())) == SEEDS, "r20 seed registry drift")
    require(protocol.get("request_count") == 30, "r20 protocol must register exactly 30 requests")
    run_root = Path(protocol["run_root"])
    require(run_root.name == "p1_factorial_r20", f"unexpected run root: {run_root}")
    ...
    for stage, expected_cells in (("preflight", set("abcd")), ("routing_probe", set("cd")), ("formal", set("abcd"))):
        ...
        for seed in SEEDS:
            ...
            for cell, entry in seed_map[str(seed)].items():
                ...
                require(protocol_path.parent in request_path.resolve().parents, f"request is outside r20 config: {request_path}")
...
def locked_runner(protocol: dict[str, Any], repo: Path, override: Path | None) -> tuple[Path, str]:
    ...
    relative = "scripts/a1/run_p1_bn_frozen_r20.py"
    ...
    if override is not None:
        require(override.resolve() == expected, f"--runner may only name the locked r20 runner: {expected}")
...
def validate_runtime_repository(protocol: dict[str, Any], repo: Path) -> None:
    ...
    require(not dirty, "r20 worktree must be clean before starting a stage")
...
def initializer_attestation(protocol: dict[str, Any], seed: int, cell: str) -> dict[str, Any]:
    ...
    require(payload.get("schema_version") == 6, f"initializer manifest schema drift: {path}")
    ...
    routers = cell_payload.get("r20_clean_aux_routers", [])
...
def request_entry(
    ...
    require(
        bool(attestation["cell"].get("r20_clean_aux_routers")) is (cell in "cd"),
...
def main() -> None:
    ...
    if stage_root.exists():
        raise FileExistsError(f"refusing to adopt or reuse r20 stage directory: {stage_root}")
