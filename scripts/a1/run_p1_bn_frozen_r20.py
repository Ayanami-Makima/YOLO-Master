#!/usr/bin/env python3
"""Run one locked r20 request with frozen BN/base and clean-aux MoE routing."""
...
R20_ROUTING_SEMANTICS = "hard_top2_from_step_zero_private_exploration_clean_aux"
R20_EXPLORATION_POLICY = {
...
R20_GAIN_POLICY = {"lr": 0.01, "weight_decay": 0.0, "warmup": False}
R20_CLEAN_AUX_POLICY = {
    "enabled": True,
...
def validate_request_registration(path: Path, request: dict) -> dict:
    ...
    if not protocol_path.is_file():
        raise ValueError(f"r20 request has no valid protocol registry: {protocol_path}")
    protocol = json.loads(protocol_path.read_text(encoding="utf-8"))
    if protocol.get("schema_version") != 6 or protocol.get("experiment_tag") != "r20":
        raise ValueError("request protocol is not r20 schema 6")
    ...
    if subprocess.check_output(["git", "status", "--porcelain"], cwd=repo, text=True).strip():
        raise ValueError("r20 worktree must be clean before executing a request")
    relative_runner = "scripts/a1/run_p1_bn_frozen_r20.py"
...
def read_request(path: Path) -> dict:
    ...
    if "p1_factorial_r19" in normalized_model or "yolo-master-r19" in normalized_model:
        raise ValueError(f"r20 request must not reference r19 weights: {model_path}")
    ...
    if routing_semantics != R20_ROUTING_SEMANTICS:
        raise ValueError(f"r20 routing semantics drift: {routing_semantics!r}")
    ...
    if not isinstance(exploration, dict):
        raise TypeError("r20 requires an explicit router_exploration policy")
    expected = {**R20_EXPLORATION_POLICY, "base_seed": params.get("seed")}
    drift = {key: (exploration.get(key), value) for key, value in expected.items() if exploration.get(key) != value}
    if drift:
        raise ValueError(f"r20 router exploration policy drift: {drift}")
    if exploration.get("enabled") not in {True, False}:
        raise ValueError("r20 router_exploration.enabled must be boolean")
    expected_aux = R20_CLEAN_AUX_POLICY if exploration["enabled"] else R20_DENSE_AUX_POLICY
    if policy.get("routing_auxiliary_objective") != expected_aux:
        raise ValueError(f"r20 routing auxiliary objective drift: {policy.get('routing_auxiliary_objective')!r}")
    gain_policy = policy.get("factor_gain_optimizer")
    if gain_policy != R20_GAIN_POLICY:
        raise ValueError(f"r20 factor gain optimizer policy drift: {gain_policy!r}")
    return request
...
def configure_r20_exploration(trainer, request: dict) -> None:
    ...
    if trainer.p1_routing_semantics != R20_ROUTING_SEMANTICS:
        return
    ...
    if enabled != bool(routed):
        raise ValueError(f"r20 exploration enabled={enabled} but routed module count is {len(routed)}")
    ...
    for index, (_, module) in enumerate(routed):
        ...
        if not sigma_min <= sigma0 <= sigma_max:
            raise ValueError(f"uncalibrated r20 router sigma0={sigma0} outside [{sigma_min}, {sigma_max}]")
        routing.configure_p1_private_noise(base_seed + stride * (index + 1), reset_step=True)
        routing.noise_std = sigma0


def r20_noise_scale(microbatch_index: int, exploration: dict) -> float:
    """Return the locked r20 hold/linear-decay multiplier for one microbatch."""
...
def enforce_and_schedule_p1_policy(trainer) -> None:
    """Reapply freeze constraints and set the r20 noise for the next train microbatch."""
    enforce_p1_freeze_policy(trainer)
    if getattr(trainer, "p1_routing_semantics", None) != R20_ROUTING_SEMANTICS:
        return
    exploration = trainer.p1_router_exploration
    scale = r20_noise_scale(trainer.p1_microbatch_index, exploration)
...
def validate_runtime_p1_policy(trainer) -> None:
    ...
    r20 = getattr(trainer, "p1_routing_semantics", None) == R20_ROUTING_SEMANTICS
    if not r20:
        raise ValueError(f"runtime routing semantics are not r20: {getattr(trainer, 'p1_routing_semantics', None)!r}")
    requested_aux = getattr(trainer, "p1_routing_auxiliary_objective", None)
    if requested_aux not in (R20_CLEAN_AUX_POLICY, R20_DENSE_AUX_POLICY):
        raise ValueError(f"runtime r20 routing-aux policy drift: {requested_aux!r}")
    clean_aux_enabled = requested_aux == R20_CLEAN_AUX_POLICY
    expected_router_count = 6 if clean_aux_enabled else 0
    if len(routed) != expected_router_count:
        raise ValueError(
            f"r20 clean_aux_enabled={clean_aux_enabled} requires {expected_router_count} routers, got {len(routed)}"
        )
    for module in routed:
        ...
        if module["p1_balance_on_clean_routes"] is not True:
            raise ValueError(f"r20 router is not balancing clean routes: {module}")
        if module["routing_aux_semantics"] != R20_CLEAN_AUX_POLICY["runtime_semantics"]:
            raise ValueError(f"r20 routing-aux semantics mismatch: {module}")
        if r20:
            ...
            if not sigma_min <= module["p1_noise_sigma0"] <= sigma_max:
                raise ValueError(f"r20 router sigma calibration mismatch: {module}")
            if not math.isclose(module["noise_std"], module["p1_noise_sigma0"], abs_tol=1e-12):
                raise ValueError(f"r20 router did not start at calibrated sigma0: {module}")
            if module["p1_noise_seed"] is None or module["p1_noise_step"] != 0:
                raise ValueError(f"r20 private router RNG was not reset: {module}")
    ...
    if r20:
        ...
        if (
            ...
            or not math.isclose(gain_groups[0]["initial_lr"], R20_GAIN_POLICY["lr"], abs_tol=1e-12)
            or gain_groups[0]["weight_decay"] != R20_GAIN_POLICY["weight_decay"]
            or not gain_groups[0]["p1_no_warmup"]
        ):
            raise ValueError(f"r20 residual gain optimizer group mismatch: {gain_groups}")
...
def main() -> None:
    ...
    def prepare_training(trainer) -> None:
        configure_r20_exploration(trainer, request)
        enforce_p1_freeze_policy(trainer)
