#!/usr/bin/env python3
"""Generate the immutable A1 P1 medium-COCO r27 residual-factor protocol.

r27 preserves r23's model, routing, initializer, freeze, runtime-source binding,
...
import yaml
from p1_r27_runtime import RUNTIME_ATTESTATION

# isort: split

from p1_r27_integrity import data_list_content_signature
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--experiment-tag", default="r27")
...
def require_locked_baseline_ancestor(repo: Path) -> None:
    """Fail unless the r27 implementation descends from the locked official baseline."""
...
def validate_destinations(repo: Path, run_root: Path, output: Path, tag: str) -> None:
    if tag != "r27":
        raise ValueError(f"r27 protocol requires experiment-tag r27, got {tag!r}")
    if repo.name != "YOLO-Master-r27-medium":
        raise ValueError(f"r27 requires isolated worktree YOLO-Master-r27-medium, got {repo}")
    expected_output = repo / "configs/a1/p1_factorial_medium_r27"
    if not same_path(output, expected_output):
        raise ValueError(f"r27 config directory must be {expected_output}, got {output}")
    if run_root.name != "p1_factorial_medium_r27":
        raise ValueError(f"r27 run root must end in p1_factorial_medium_r27, got {run_root}")
...
def calibration_images(formal_data: Path) -> tuple[list[str], Path]:
    ...
    if not isinstance(train, str):
        raise TypeError("r27 requires formal train to be one explicit image-list file")
    ...
    if len(lines) < 512:
        raise ValueError(f"r27 BN calibration needs 512 train images, found {len(lines)}")
    resolved: list[str] = []
...
def data_registry_entry(
    ...
    for split, expected_count in (("train", expected_train), ("val", expected_val)):
        ...
        if not isinstance(entry, str):
            raise TypeError(f"r27 requires {data_yaml} {split} to be one explicit image-list file")
...
def main() -> None:
    ...
    if python != EXPECTED_PYTHON:
        raise ValueError(f"r27 requires the exact virtualenv launcher {EXPECTED_PYTHON}, got {python}")
    if RUNTIME_ATTESTATION["interpreter"]["executable"] != str(python):
        raise RuntimeError("r27 protocol generator was not launched by the registered virtualenv interpreter")
    if RUNTIME_ATTESTATION["interpreter"]["virtualenv"] is not True:
        raise RuntimeError("r27 protocol generation requires an active virtualenv interpreter")
    forbid_r19_artifact(checkpoint, "source checkpoint")
    if checkpoint != OFFICIAL_CHECKPOINT.resolve():
        raise ValueError(f"r27 must start from the locked official checkpoint {OFFICIAL_CHECKPOINT}, got {checkpoint}")
    if sha256(checkpoint) != OFFICIAL_CHECKPOINT_SHA256:
        raise ValueError("official yolo26n.pt SHA-256 mismatch")
    branch = git_output(repo, "branch", "--show-current")
    if branch != "a1-p1-medium-r27":
        raise ValueError(f"r27 worktree branch must be a1-p1-medium-r27, got {branch!r}")
    require_locked_baseline_ancestor(repo)
    dirty = git_output(repo, "status", "--porcelain")
    if dirty:
        raise ValueError("implementation worktree must be clean before generating the r27 protocol")
    ...
    implementation_paths = (
        ...
        *moe_runtime_paths,
        ...
        repo / "scripts/a1/prepare_p1_residual_factor_r27.py",
        repo / "scripts/a1/prepare_p1_medium_data_r27.py",
        repo / "scripts/a1/p1_r27_integrity.py",
        repo / "scripts/a1/p1_r27_runtime.py",
        repo / "scripts/a1/prepare_p1_factorial.py",
        repo / "scripts/a1/build_p1_residual_factor_initializers_r27.py",
        repo / "scripts/a1/build_p1_residual_factor_initializers.py",
        repo / "scripts/a1/run_p1_bn_frozen_r27.py",
        repo / "scripts/a1/run_p1_factorial_multiseed_2gpu_r27.py",
        repo / "scripts/a1/audit_p1_checkpoints_r27.py",
        repo / "scripts/a1/audit_p1_residual_factor_checkpoints.py",
        repo / "scripts/a1/audit_p1_routing_r27.py",
        repo / "scripts/a1/audit_p1_preflight_routing.py",
        repo / "scripts/a1/audit_p1_routing.py",
        repo / "scripts/a1/evaluate_p1_matrix.py",
        repo / "scripts/a1/audit_p1_residual_activity_r27.py",
        repo / "scripts/a1/audit_p1_formal_admission_r27.py",
        repo / "scripts/a1/verify_p1_r27_migration.py",
        repo / "tests/test_p1_residual_factor.py",
        repo / "tests/test_moe_router_boundaries.py",
        repo / "tests/test_peft_optimizer_policy.py",
        repo / "tests/test_p1_r27_protocol.py",
    )
    ...
    protocol: dict[str, Any] = {
        "schema_version": 8,
        "name": "A1 P1 medium-COCO equal-budget 2x2 factorial r27 residual-factor",
        "experiment_tag": "r27",
        ...
        "r27_medium_coco_extension": {
            "scope": "medium-COCO formal budget plus user-authorized shared-GPU execution; authorized before any r27 training",
        ...
        "routing_probe_policy": "C/D x 3 seeds start independently from original r27 initializers; discard all weights",
        ...
        "gpu_policy": {
            "mode": "deterministic memory-aware independent jobs on GPU0/GPU1; never DDP; request-visible device is logical 0",
            "ordered_job_queue": "seed ascending, then A/B/C/D; routing probes seed ascending then C/D",
            "assignment": "each wave fills GPU0 then GPU1 up to the capacity derived from the stage-start free-memory snapshot",
            "co_location": {
                "authorized": True,
                "authorization": "user explicitly clarified memory-aware shared-GPU concurrency on 2026-08-30",
                "one_job_minimum_free_memory_mib": 12000,
                "two_jobs_minimum_free_memory_mib": 22000,
                "maximum_jobs_per_gpu": 2,
