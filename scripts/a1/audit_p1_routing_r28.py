#!/usr/bin/env python3
"""Apply the A1-aligned full-val5000 hard Top-2 diagnostics to r28 C/D probes.
    ...
    for name, module in model.model.named_modules():
        ...
        if routing is not None and hasattr(routing, "num_experts") and hasattr(routing, "top_k"):
            if getattr(routing, "p1_balance_on_clean_routes", False) is not True:
                raise ValueError(f"r28 clean-aux flag missing: {name}.routing")
            if getattr(module, "routing_aux_semantics", None) != R28_CLEAN_AUX_POLICY["runtime_semantics"]:
                raise ValueError(f"r28 routing-aux semantics mismatch: {name}.routing")
...
def main() -> None:
    ...
    if protocol.get("schema_version") != 8 or protocol.get("experiment_tag") != "r28":
        raise ValueError("r28 routing audit requires an r28 schema-8 protocol")
    ...
    if output.exists():
        raise FileExistsError("refusing to overwrite r28 routing evidence")
