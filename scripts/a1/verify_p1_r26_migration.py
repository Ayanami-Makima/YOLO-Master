#!/usr/bin/env python3
"""Verify r26 implementation identity, A/B invariance, and C/D policy parity."""
...
from run_p1_bn_frozen_r26 import R26_CLEAN_AUX_POLICY
...
def verify_git_lineage(repo: Path, protocol: dict[str, Any]) -> dict[str, str]:
    ...
    require(not dirty, "r26 worktree is dirty during migration audit")
...
def main() -> None:
    ...
    # r26 deliberately recalibrates factor BatchNorm on the first 512 medium-COCO
    ...
    for key in semantic_keys:
        ...
        require(equal, f"r26 semantic drift from r23: {key}")
        semantic_comparison[key] = True
    for key in (
        "seed_to_physical_gpu",
        "within_seed_order",
        "routing_probe_within_seed_order",
        "gpu0_seed_order",
        "gpu1_seed_order",
    ):
        require(protocol["gpu_policy"].get(key) == r23["gpu_policy"].get(key), f"r26 GPU schedule drift: {key}")
    require(protocol["gpu_policy"].get("co_location", {}).get("authorized") is True, "shared-GPU authorization missing")
    for cell in "abcd":
        current = protocol["configs"][cell]
        previous = r23["configs"][cell]
        for key in ("moe", "end2end", "sha256"):
            require(current.get(key) == previous.get(key), f"r26 config semantic drift: {cell}/{key}")
    for key in ("images", "batch", "imgsz", "augment", "shuffle", "grad", "post_calibration_training_policy"):
        require(protocol["batch_norm"].get(key) == r23["batch_norm"].get(key), f"r26 BN policy drift: {key}")
    ...
    for seed in SEEDS:
        ...
        for cell in "ab":
            r26_path = Path(protocol["run_root"]) / "initializers" / f"seed{seed}" / f"{cell}_residual_factor_init.pt"
            require(r26_path.is_file(), f"missing A/B initializer: {seed}/{cell}")
            ...
            require(passed, f"r26 A/B invariance failed: {seed}/{cell}")
            comparisons[str(seed)][cell] = {
                "r26": str(r26_path),
                "r26_sha256": sha256(r26_path),
        ...
        for cell in "cd":
            ...
            require(
                all(item[3] and item[4] == R26_CLEAN_AUX_POLICY["runtime_semantics"] for item in signatures[cell]),
