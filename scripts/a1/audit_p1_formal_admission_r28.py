#!/usr/bin/env python3
"""Compose the fail-closed r28 formal-admission evidence."""
...
def validate_implementation(protocol: dict[str, Any]) -> None:
    ...
    require(not dirty, "r28 worktree is dirty at formal admission")
...
def validate_stage_execution(protocol: dict[str, Any], protocol_path: Path, stage: str, cells: str) -> dict[str, str]:
    ...
    runtime_relative = "scripts/a1/p1_r28_runtime.py"
    require(
    ...
    require(
        ...
        and shared_policy.get("maximum_jobs_per_gpu") == 1,
    ...
    relative_runner = "scripts/a1/run_p1_bn_frozen_r28.py"
...
def validate_initializers(protocol: dict[str, Any], protocol_path: Path) -> tuple[list[dict[str, str]], bool]:
    ...
    for seed in SEEDS:
        ...
        for cell in CELLS:
            ...
            clean_aux_routers = item.get("r28_clean_aux_routers")
            if cell in "ab":
...
def main() -> None:
    ...
    protocol = read_json(protocol_path, "r28 protocol")
    assert_protocol_runtime(protocol)
    protocol_sha = sha256(protocol_path)
    require(protocol.get("schema_version") == 8 and protocol.get("experiment_tag") == "r28", "not r28 schema8")
    ...
    require(
        ...
        "r28 must not reuse r23/r24/r25 initializer or trained weights",
    )
