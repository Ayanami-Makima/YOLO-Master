#!/usr/bin/env python3
"""Generate the immutable A1 P1 r20 three-seed residual-factor protocol.

r20 changes one MoE-only training detail relative to r19: noisy hard-Top2
...
import yaml
from p1_r20_integrity import data_list_content_signature
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--experiment-tag", default="r20")
...
def require_locked_baseline_ancestor(repo: Path) -> None:
    """Fail unless the r20 implementation descends from the locked official baseline."""
...
def validate_destinations(repo: Path, run_root: Path, output: Path, tag: str) -> None:
    if tag != "r20":
        raise ValueError(f"r20 protocol requires experiment-tag r20, got {tag!r}")
    if repo.name != "YOLO-Master-r20":
        raise ValueError(f"r20 requires isolated worktree YOLO-Master-r20, got {repo}")
    expected_output = repo / "configs/a1/p1_factorial_r20"
    if not same_path(output, expected_output):
        raise ValueError(f"r20 config directory must be {expected_output}, got {output}")
    if run_root.name != "p1_factorial_r20":
        raise ValueError(f"r20 run root must end in p1_factorial_r20, got {run_root}")
...
def calibration_images(pilot_data: Path) -> tuple[list[str], Path]:
    ...
    if not isinstance(train, str):
        raise TypeError("r20 requires pilot train to be one explicit image-list file")
    ...
    if len(lines) < 512:
        raise ValueError(f"r20 BN calibration needs 512 train images, found {len(lines)}")
    resolved: list[str] = []
...
def data_registry_entry(data_yaml: Path, *, expected_train: int, expected_val: int) -> dict[str, Any]:
    ...
    for split, expected_count in (("train", expected_train), ("val", expected_val)):
        ...
        if not isinstance(entry, str):
            raise TypeError(f"r20 requires {data_yaml} {split} to be one explicit image-list file")
...
def main() -> None:
    ...
    if checkpoint != OFFICIAL_CHECKPOINT.resolve():
        raise ValueError(f"r20 must start from the locked official checkpoint {OFFICIAL_CHECKPOINT}, got {checkpoint}")
    if sha256(checkpoint) != OFFICIAL_CHECKPOINT_SHA256:
        raise ValueError("official yolo26n.pt SHA-256 mismatch")
    branch = git_output(repo, "branch", "--show-current")
    if branch != "a1-p1-r20":
        raise ValueError(f"r20 worktree branch must be a1-p1-r20, got {branch!r}")
    require_locked_baseline_ancestor(repo)
    dirty = git_output(repo, "status", "--porcelain")
    if dirty:
        raise ValueError("implementation worktree must be clean before generating the r20 protocol")
    ...
    implementation_paths = (
        ...
        *moe_runtime_paths,
        ...
        repo / "scripts/a1/prepare_p1_residual_factor_r20.py",
        repo / "scripts/a1/p1_r20_integrity.py",
        repo / "scripts/a1/prepare_p1_factorial.py",
        repo / "scripts/a1/build_p1_residual_factor_initializers_r20.py",
        repo / "scripts/a1/build_p1_residual_factor_initializers.py",
        repo / "scripts/a1/run_p1_bn_frozen_r20.py",
        repo / "scripts/a1/run_p1_factorial_multiseed_2gpu_r20.py",
        repo / "scripts/a1/audit_p1_checkpoints_r20.py",
        repo / "scripts/a1/audit_p1_residual_factor_checkpoints.py",
        repo / "scripts/a1/audit_p1_routing_r20.py",
        repo / "scripts/a1/audit_p1_preflight_routing.py",
        repo / "scripts/a1/audit_p1_routing.py",
        repo / "scripts/a1/evaluate_p1_matrix.py",
        repo / "scripts/a1/audit_p1_residual_activity_r20.py",
        repo / "scripts/a1/audit_p1_formal_admission_r20.py",
        repo / "scripts/a1/verify_p1_r20_migration.py",
        repo / "tests/test_p1_residual_factor.py",
        repo / "tests/test_moe_router_boundaries.py",
        repo / "tests/test_peft_optimizer_policy.py",
        repo / "tests/test_p1_r20_protocol.py",
    )
    ...
    protocol: dict[str, Any] = {
        "schema_version": 6,
        "name": "A1 P1 equal-budget 2x2 factorial r20 residual-factor",
        "experiment_tag": "r20",
        ...
        "routing_probe_policy": "C/D x 3 seeds start independently from original r20 initializers; discard all weights",
