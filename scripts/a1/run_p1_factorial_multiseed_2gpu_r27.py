#!/usr/bin/env python3
"""Run immutable r27 stages with deterministic memory-aware two-GPU waves.

A GPU with at least 22 GiB free admits two independent jobs; one with at least
12 GiB free admits one. Requests still see logical device 0 through
CUDA_VISIBLE_DEVICES, and every observed capacity and assignment is recorded.
"""
...
def stage_schedule(
    ...
    if sum(capacities.get(index, 0) for index in GPU_INDICES) < 1:
        raise ValueError("neither GPU has enough free memory for one r27 job")
    while pending:
...
def validate_protocol(protocol: dict[str, Any], protocol_path: Path) -> None:
    require(protocol.get("schema_version") == 8, "r27 runner requires protocol schema 8")
    require(protocol.get("experiment_tag") == "r27", "runner protocol is not r27")
    require(tuple(protocol.get("seeds", ())) == SEEDS, "r27 seed registry drift")
    require(protocol.get("request_count") == 30, "r27 protocol must register exactly 30 requests")
    run_root = Path(protocol["run_root"])
    require(run_root.name == "p1_factorial_medium_r27", f"unexpected run root: {run_root}")
    ...
    require(
        ...
        == {
            ...
            "two_jobs_minimum_free_memory_mib": TWO_JOB_FREE_MEMORY_MIB,
    ...
    for stage, expected_cells in (("preflight", set("abcd")), ("routing_probe", set("cd")), ("formal", set("abcd"))):
        ...
        for seed in SEEDS:
            ...
            for cell, entry in seed_map[str(seed)].items():
                ...
                require(protocol_path.parent in request_path.resolve().parents, f"request is outside r27 config: {request_path}")
...
def locked_runner(protocol: dict[str, Any], repo: Path, override: Path | None) -> tuple[Path, str]:
    ...
    if override is not None:
        require(override.resolve() == expected, f"--runner may only name the locked r27 runner: {expected}")
...
def validate_runtime_repository(protocol: dict[str, Any], repo: Path) -> None:
    ...
    require(not dirty, "r27 worktree must be clean before starting a stage")
...
def validate_child_interpreter(protocol: dict[str, Any], repo: Path, requests: list[dict]) -> dict:
    """Prove the exact registered child Python can import torch and r27."""
    ...
    runtime_script = repo / "scripts/a1/p1_r27_runtime.py"
...
def initializer_attestation(protocol: dict[str, Any], seed: int, cell: str) -> dict[str, Any]:
    ...
    routers = cell_payload.get("r27_clean_aux_routers", [])
...
def request_entry(
    ...
    require(
        bool(attestation["cell"].get("r27_clean_aux_routers")) is (cell in "cd"),
...
def main() -> None:
    ...
    if stage_root.exists():
        raise FileExistsError(f"refusing to adopt or reuse r27 stage directory: {stage_root}")
